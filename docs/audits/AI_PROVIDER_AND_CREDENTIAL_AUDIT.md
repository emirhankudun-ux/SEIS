...
Inspected files: 2429

## Provider Matrix

| Provider | Category | Status | Expected env vars | Locations | Frontend direct | Backend side | Decision | Recommended action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Anthropic Claude | cloud model provider | Live but Unverified | ANTHROPIC_API_KEY | 4939 | no | yes | Retain | Retain as unverified until provider health, no-key startup, and redaction tests exist. |
...
| Google Gemini | cloud model provider | Live but Unverified | GEMINI_API_KEY | 114 | no | yes | Retain | Retain as unverified until provider health, no-key startup, and redaction tests exist. |
...
| Hugging Face | model hosting provider | Unknown | HF_TOKEN | 11 | no | no | Retain | Review manually before enabling. |
| LM Studio | local model provider | Unknown | LM_STUDIO_BASE_URL | 6 | no | no | Retain | Review manually before enabling. |
...
| Ollama | local model provider | Live but Unverified | OLLAMA_BASE_URL, OLLAMA_HOST | 325 | no | yes | Retain | Retain as unverified until provider health, no-key startup, and redaction tests exist. |
...
| Replicate | media/model provider | Unknown | REPLICATE_API_TOKEN | 64 | no | no | Retain | Review manually before enabling. |
| Runway | media provider | Unknown | RUNWAY_API_KEY | 288 | no | no | Retain | Review manually before enabling. |
| Stability AI | image provider | Unknown | STABILITY_API_KEY | 1 | no | no | Retain | Review manually before enabling. |
| Together AI | cloud model provider | Unknown | TOGETHER_API_KEY | 92 | no | no | Retain | Review manually before enabling. |
...
| CLOUDFLARE_API_TOKEN | scripts/check-seis-public-readiness-sensitive-boundary.mjs | 53 | backend |
...
| GITHUB_TOKEN | scripts/check-seis-public-readiness-sensitive-boundary.mjs | 53 | backend |
...
| NETLIFY_AUTH_TOKEN | scripts/check-seis-public-readiness-sensitive-boundary.mjs | 53 | backend |
...
| VERCEL_TOKEN | scripts/check-seis-public-readiness-sensitive-boundary.mjs | 53 | backend |
...
- Stability AI: STABILITY_API_KEY (Unknown)
