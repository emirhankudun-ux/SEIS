...
The first structured performed review log is
[`../../content/development/seis-goal-review-log.json`](../../content/development/seis-goal-review-log.json).
It records actual daily, weekly, or monthly review occurrences separately from
the recurring cadence definitions.

| Field | Required | Description |
| --- | --- | --- |
| `id` | Yes | Stable review log id such as `SEIS-REVIEW-LOG-001`. |
| `title` | Yes | Short review title. |
| `cadence` | Yes | `daily`, `weekly`, or `monthly`. |
| `status` | Yes | `performed`, `partial`, or `blocked`. |
| `review_date` | Yes | Date the review was performed. |
| `owner_role` | Yes | Responsible role. |
| `related_review_id` | Yes | Cadence record id, such as `SEIS-REVIEW-001`. |
| `related_goal_ids` | Yes | Goal ids reviewed. |
| `related_task_ids` | Yes | Task ids reviewed. |
| `evidence_ids` | Yes | Evidence proving the review occurrence. |
| `related_paths` | Yes | Repo-relative paths. |
| `what_changed` | Yes | Current-period changes observed. |
| `active_blockers` | Yes | Blockers visible during the review. |
| `validation_performed` | Yes | Validation run for the review. |
| `validation_needed` | Yes | Validation still needed. |
| `limitations` | Yes | What the review does not prove. |
| `next_safe_action` | Yes | Next safe action after the review. |

Review log records are validated by `npm run check:goal-tracking`.
...
## Objective Coverage Object

The first structured objective coverage registry is
[`../../content/development/seis-goal-objective-coverage.json`](../../content/development/seis-goal-objective-coverage.json).
It maps the Goal Tracking OS mission requirements to evidence, limitations, and
next safe actions.

| Field | Required | Description |
| --- | --- | --- |
| `id` | Yes | Stable id such as `SEIS-OBJ-001`. |
| `objective_section` | Yes | Mission section being reviewed. |
| `requirement` | Yes | Requirement being covered. |
| `status` | Yes | `passed`, `partial`, `blocked`, `planned`, or `unknown`. |
| `evidence_ids` | Yes | Evidence records supporting the coverage status. |
| `related_goal_ids` | Yes | Goal ids connected to the requirement. |
| `related_paths` | Yes | Repo-relative files or directories. |
| `proof` | Yes | What current repository evidence proves. |
| `limitations` | Yes | What the evidence does not prove. |
| `next_action` | Yes | Next safe action for the requirement. |

Objective coverage records are validated by `npm run check:goal-tracking`.

## Completion Gate Object

The structured completion gate is
[`../../content/development/seis-goal-completion-gate.json`](../../content/development/seis-goal-completion-gate.json).
It distinguishes proved foundation work from partial, blocked, and unverified
full-objective requirements.

| Field | Required | Description |
| --- | --- | --- |
| `schemaVersion` | Yes | Completion gate schema version. |
| `id` | Yes | Stable gate id, currently `seis-goal-completion-gate`. |
| `mode` | Yes | Must be `non_llm_goal_completion_gate`. |
| `finalDecision` | Yes | `complete`, `not_complete`, `blocked_by_repository_hygiene`, or `blocked_by_missing_evidence`. |
| `records` | Yes | Requirement gate records. |

### Gate Record Fields

| Field | Required | Description |
| --- | --- | --- |
| `id` | Yes | Stable id such as `SEIS-GATE-001`. |
| `requirement` | Yes | Requirement being audited. |
| `status` | Yes | `proved`, `partial`, `blocked`, or `unverified`. |
| `evidence_ids` | Yes | Evidence records supporting the gate status. |
| `related_goal_ids` | Yes | Goal ids connected to the requirement. |
| `related_paths` | Yes | Repo-relative files or directories. |
| `proof` | Yes | What current evidence proves. |
| `gap` | Yes | What remains missing, partial, blocked, or unverified. |
| `next_action` | Yes | Next safe action for the gate. |

Completion gate records are validated by `npm run check:goal-tracking`.
...
| `panels.actualReviews` | Yes | Performed review log records with evidence. |
...
| `panels.objectiveCoverage` | Yes | Mission requirements mapped to evidence and limitations. |
| `panels.completionGate` | Yes | Strict completion gate records and final decision. |
