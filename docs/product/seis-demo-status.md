...
| SEIS Website | Static product hub and product pages for AI, OS, Code, Design, Data, Search, Cloud, Store, and Agents | `apps/web/website/`, `apps/web/desktop.js`, `scripts/check-seis-website-pages.mjs`, `npm run check:seis-website-pages` |
...
| Command Center GitHub merge gates | Browser-local protected-branch status panel with PR #58, #62, and #65 review-gate evidence plus 10-review, code-owner, last-push, signed-commit, linear-history, and CodeQL gate labels | Live GitHub approval, merge, push, admin bypass, branch-protection change, or claim that auto-merge bypasses human review |
