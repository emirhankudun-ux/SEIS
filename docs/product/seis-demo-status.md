...
| SEIS Linux Replica | Standalone browser-smoked local route | `apps/web/seis-linux-replica.html`, `npm run check:seis-linux-replica-browser-smoke`; verifies boot/login, SEIS System OS top bar, quick app controls, pinned side rail, five activity cards, safe session persistence, 286 launcher targets, 67 audited functional app workflows, 35 enhanced workbench snapshot/reset flows, 8 playable local games, terminal `neofetch`, terminal `apps`, terminal `seis` bridge listing, connected Search/Code/Design/Cloud/Website result cards, mini SEIS Code/Design/Cloud/Store/Music/AI workspaces with local state actions, local-only SSH/host-shell boundary, resizable/draggable windows, VFS, and screenshot evidence. |
