...
| SEIS Linux Replica | Standalone browser-smoked local route | `apps/web/seis-linux-replica.html`, `npm run check:seis-linux-replica-browser-smoke`; verifies boot/login, SEIS System OS top bar, quick app controls, pinned side rail, five activity cards, safe session persistence, 65+ app launcher plus supplied references, terminal `neofetch`, terminal `seis` bridge listing, terminal `mcp` Plugin/MCP lane listing, connected Search/Code/Design/Cloud/Website/Plugin result cards, SEIS Store package marketplace rows for apps/plugins/MCP workbench lanes/installed AI lanes/themes/developer tools/design tools/governance packs with browser-local install/update/enable state, Plugin/MCP Workbench lane filters, installed-AI markers, subagent dry-run queue, permission matrix, mini SEIS Code/Design/Cloud/Store/Music/AI workspaces with local state actions, local-only SSH/host-shell boundary, resizable/draggable windows, VFS, and screenshot evidence. |
...
| Linux Replica Store package marketplace | Browser-local Store package rows for developer tools, design tools, AI agents, plugins, MCP workbench lanes, installed AI lanes, themes, DevOps guardrails, Reference Vault, and Search Gateway with persisted local install/update/enable state | Live marketplace publication, billing, dependency installation, external connector authorization, provider calls, host package installation, SSH, deployment, GitHub mutation, or proof that a package is production-installed |
| Linux Replica Plugin/MCP Workbench | Browser-local Plugin/MCP/installed-AI lane review app with filters, lane focus persistence, subagent dry-run queue, permission matrix, terminal `mcp` command, and VFS review-note export | Connector authentication, live MCP trust, provider calls, autonomous writes, host execution, dependency install, SSH, deployment, GitHub mutation, push, merge, release, or proof that any external tool was actually invoked |
