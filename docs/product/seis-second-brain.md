...
| Plugin + skill graph nodes | Browser-local Local Demo | The Second Brain graph now renders 5 plugin/skill readiness nodes for `@seis`, `@seis-cloud`, `@seis-code`, `@seis-design`, and `@seis-data`, plus local graph-edge markers that connect the readiness matrix back to SEIS AI. The nodes open the SEIS AI bridge and remain review-only; no plugin, MCP, provider, SSH, deployment, or GitHub action is executed. |
...
| SEIS AI plugin/skill bridge | Local Demo review-only | The SEIS AI Second Brain tab now renders the Plugin + Skill Readiness matrix with all 5 personal plugin lanes, status/plan tools, embedded `SKILL.md` paths, training-use text, `providerExecution false`, and `externalMutation false`, so AI Core sees the same readiness boundary as the Second Brain contract. |
...
| Obsidian Safe Import Selector | Runtime review-only | SEIS Second Brain now renders a planned-gated source-mode selector and metadata-only dry-run manifest preview for the Obsidian bridge. `Prepare Safe Import Dry-Run` writes `/home/seis/SecondBrain/obsidian-safe-import-ui-dry-run.md` in browser VFS only, keeps `selectedByUser: false`, blocks host filesystem scans, and does not import private note bodies. |
