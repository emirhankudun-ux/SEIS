...
| Agent review queue | Browser-local review-only | `Build Agent Review Queue` writes `/home/seis/SecondBrain/09-review/agent-review-queue.md` with all 13 plan-only agent duties, the 6 installed AI profile states, and references to the all-lane bundle, explicit-selection receipt, preflight request, safe-import dry-run, and agent-registry report paths. It does not inspect those artifacts, scan a host folder, access a private vault, invoke a provider/MCP/SSH connection, or authorize autonomous execution. |
