...
| 1B | `docs: add SEIS Second Brain readiness contracts` | Keep the Obsidian bridge safe import, Second Brain accessibility/focus QA, read-only model-router contract, and PR #54 public demo release checklist connected to docs, structured records, and validators before public demo review. | `npm run check:seis-second-brain-readiness-contracts`, `npm run check:seis-second-brain`, `npm run check:seis-second-brain-browser-smoke`, `git diff --check` | None for docs, JSON contracts, accessibility markers, and local validators; approval required for private Obsidian import, live provider routing, SSH, deployment, merge, Pages publication, or public release. |
...
| 5 | `docs: add SEIS Design Agency Kit foundation` | Keep the design lane useful for graphic-design and agency-style work by validating editable creative brief, client discovery intake matrix, brand and offer naming matrix, brand strategy workshop matrix, landing page blueprint matrix, agency cost control matrix, agency cost defense calculator, creative asset licensing board, design sprint timeline matrix, proposal scope estimator, agency quote comparator, competitive positioning matrix, brand voice messaging matrix, typography hierarchy matrix, color system accessibility matrix, print production readiness matrix, brand rationale deck, visual reference moodboard, creative asset shot list matrix, logo concept evaluation matrix, brand usage guideline, creative director QA, design review decision matrix, approval state transition ledger, revision round plan, client feedback triage board, case study layout board, visual QA evidence ledger, production file manifest, asset size spec sheet, client approval packet, client-ready export index, brand-token, landing direction, campaign, asset provenance, presentation, brand audit scorecard, launch asset matrix, social content calendar matrix, social variant set, presentation system map, visible Agency Workboards, browser-local draft pack, SEIS Code handoff export, SEIS Code review note creation, and handoff outputs without API keys or live-provider claims. | `npm run check:seis-design-agency-kit`, `npm run check:seis-code`, `npm run check:seis-design-agency-kit-browser-smoke`, `npm run check:design-component-inventory`, `npm run check:seis-website-pages`, `npm run check:seis-public-readiness`, `git diff --check` | None for docs, JSON, browser-local UI, and local validators; approval required for client publication, paid media, legal/trademark review, dependencies, live providers, deployment, SSH, push, merge, or Pages publication. |
...
| Validation | `npm run check:seis-second-brain-readiness-contracts`, `npm run check:seis-second-brain`, `npm run check:seis-second-brain-browser-smoke`, `node --check apps/web/desktop.js`, `node --check scripts/check-seis-second-brain-readiness-contracts.mjs`, `git diff --check`. |
...
| --- | --- |
| Priority | P1 |
| Goal | Reconcile legacy UIXAppTTR-era branch wording, `SEIS-BL-035` release artifact retention, archive-ledger coverage, and `SEIS-BL-037` backlog ID validation. |
| Include | Branch-policy wording review, `docs/release-artifact-policy` retention policy, archive-ledger records for assistant materials, and the `docs/backlog-id-validator` uniqueness check. |
...
| Validation | Docs review, `npm run check:branch-policy-reconciliation`, `npm run check:release-artifact-policy`, `npm run check:master-backlog`, `git diff --check`. |
