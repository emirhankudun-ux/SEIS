...
The Obsidian graph map belongs in the same lane: it turns repo-owned seed
notes, starter vault artifacts, the AI council review pack, and GitHub
readiness gates into a browser-local wikilink entry note without private vault
reads, plugin install, private note body copy, provider calls, autonomous
writes, SSH, deployment, GitHub mutation, merge, or release approval.
The Agent training drills pack belongs in the same lane: it turns all
registered AI profiles, managed sub-agent lanes, and the 12-agent roster into
repeatable Obsidian graph, registry, and release-gate exercises without
provider login, private vault reads, plugin install, autonomous writes, SSH,
...
| 1B | `docs: add SEIS Second Brain readiness contracts` | Keep the Obsidian bridge safe import, Obsidian graph map, Agent training drills, Obsidian safe-import dry-run artifact, read-only model-router decision artifact, accessibility/focus QA artifact, Second Brain agent registry artifact, browser-local public contributor onboarding pack, Second Brain public reviewer pack, public demo security gate redacted evidence, security owner handoff, PR #127 security remediation plan, Second Brain accessibility/focus QA, read-only model-router contract, PR #54 public demo release checklist, PR #54 review packet, dirty worktree review, and stage plan connected to docs, structured records, validators, and the SEIS public demo go/no-go gate before public demo review. | `npm run report:seis-obsidian-safe-import-dry-run`, `npm run check:seis-obsidian-safe-import-dry-run`, `npm run report:seis-read-only-model-router-decision`, `npm run check:seis-read-only-model-router-decision`, `npm run report:seis-second-brain-accessibility-focus-report`, `npm run check:seis-second-brain-accessibility-focus-report`, `npm run report:seis-second-brain-agent-registry`, `npm run check:seis-second-brain-agent-registry`, `npm run report:seis-second-brain-public-reviewer-pack`, `npm run check:seis-second-brain-public-reviewer-pack`, `npm run report:seis-public-demo-security-gate`, `npm run check:seis-public-demo-security-gate`, `npm run report:seis-security-owner-handoff`, `npm run check:seis-security-owner-handoff`, `npm run check:seis-second-brain-readiness-contracts`, `npm run check:seis-second-brain`, `npm run check:seis-public-demo-go-no-go -- --run-fast-checks`, `npm run report:seis-public-demo-go-no-go`, `npm run check:seis-second-brain-browser-smoke`, `git diff --check` | None for docs, JSON contracts, browser-local public contributor pack generation, Obsidian graph map generation, Agent training drills generation, accessibility markers, local validators, read-only NO-GO classification, Obsidian safe-import dry-run artifact generation, read-only model-router decision artifact generation, accessibility/focus QA artifact generation, Second Brain agent registry artifact generation, Second Brain public reviewer pack generation, redacted security gate artifact generation, security owner handoff generation, PR #127 plan-only remediation plan, PR #54 review packet generation, dirty worktree review generation, and stage plan generation; approval required for private Obsidian import, live provider routing, autonomous agent write execution, SSH, deployment, `.gitleaks.toml` security-policy changes, history rewrite, force push, merge, Pages publication, public release, staging, commit, or push. |
...
| 1H | `feat: add Linux Replica Store package marketplace` | Make the Linux Replica Store a browser-local package surface for apps, plugins, MCP workbench lanes, installed AI lanes, themes, developer tools, design tools, DevOps guardrails, Reference Vault, and Search Gateway with persisted install/update/enable state. | `node --check scripts/check-seis-linux-replica-browser-smoke.mjs`, `npm run check:seis-linux-replica-browser-smoke`, `npm run check:seis-ultimate-demo`, `git diff --check` | None for browser-local package rows, category filters, localStorage state, docs, and smoke coverage; approval required for live marketplace publication, billing, dependency installation, connector authorization, provider calls, host package installs, SSH, deployment, GitHub writes, push, merge, tags, or release. |
...
## PR 4F.15: Plugin/MCP Ten-Year Continuity

| Field | Value |
| --- | --- |
| Suggested branch | `seis/command-center-plugin-mcp-continuity` |
| Priority | P1 |
| Goal | Surface a Command Center continuity map that turns installed AI, plugin, connector, and MCP inventory into a ten-year supervised planning cadence without activating live connectors or remote MCP trust. |
| Include | `apps/web/desktop.js`, `scripts/check-desktop-os.mjs`, `content/development/seis-plugin-mcp-ten-year-continuity-map.json`, `scripts/create-seis-plugin-mcp-ten-year-continuity-map.mjs`, `package.json`, `docs/platform/seis-plugin-mcp-ten-year-continuity-map.md`, `reports/seis-public-demo/plugin-mcp-ten-year-continuity-map-latest.json`, `reports/seis-public-demo/plugin-mcp-ten-year-continuity-map-latest.md`, `docs/STATUS.md`, `docs/roadmap/NEXT_PR_QUEUE.md`, browser-VFS `/home/seis/Documents/seis-plugin-mcp-ten-year-continuity-map.md` export evidence. |
| Exclude | Live connector writes, remote MCP trust, provider credentials, external AI file/prompt sends, plugin installation/publication, SSH, deployment, billing/cloud spend, GitHub push/merge/tag/release, or claims that ten years of autonomous execution already happened. |
| Validation | `node --check apps/web/desktop.js`, `node --check scripts/check-desktop-os.mjs`, `node --check scripts/create-seis-plugin-mcp-ten-year-continuity-map.mjs`, `npm run report:seis-plugin-mcp-ten-year-continuity-map`, `npm run check:seis-plugin-mcp-ten-year-continuity-map`, `npm run check:desktop-os`, `git diff --check`. |
| Approval needed | None for browser-local UI, diagnostics, VFS export, docs, and validator work; approval required for every excluded live/runtime action. |
...
- PR #104 security scan remediation: local redacted current-tree gitleaks scan is clean after replacing the static web test i18n fixture value in `apps/web/test/scripts.test.js` with the real weekly usage translation-key shape; full-history scan still reports redacted historical findings in `sources/github-unified-source/_generated/github-code-bundle.txt` at commit `f3d385d6039f370de514b3dbc2907b91a96b1572` and the older web test fixture commit `31266027d7c5d91d3b9f56d790d6d4ccd59a7672`; `reports/seis-public-demo/security-owner-handoff-latest.md` now records owner decisions and agent assignments; do not print values, do not blanket-allowlist the bundle, and require explicit human approval for history rewrite, affected-secret rotation, `.gitleaks.toml` security-policy changes, force push, release override, or a reviewed security baseline.
- PR #127 active security gate impact: `reports/seis-public-demo/security-gate-redacted-latest.md`, `reports/seis-public-demo/security-owner-handoff-latest.md`, and `docs/security/PR127_SECURITY_REMEDIATION_PLAN.md` must keep PR #127 listed as blocked by `Secret & Vulnerability Scan` and `Security Summary` until the full-history remediation path is owner-approved and completed; no merge, public demo release, Pages publication, history rewrite, `.gitleaks.toml` scanner-policy change, force push, or release override is allowed from this Second Brain readiness PR without that approval.
