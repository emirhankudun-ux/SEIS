...
| 1D.1 | `docs: add NVIDIA AI-Q runtime allowlist plan` | Keep AI-Q Deploy and AI-Q Research selected as the first planned/gated NVIDIA runtime candidate, with explicit clone/env/backend/query/credential approval gates before any live action. | `npm run check:seis-nvidia-aiq-runtime-allowlist-plan`, `npm run check:seis-nvidia-installed-integrations`, `npm run check:seis-nvidia-accelerator-catalog`, `npm run plan:nvidia-catalog-install`, `git diff --check` | None for JSON plan, docs, review packet, Desktop metadata surface, and validation; approval required for clone, `deploy/.env`, Docker, Python, Node, Kubernetes, Helm, port binding, endpoint calls, research queries, credentials, SSH, cloud, deployment, push, merge, or release. |
