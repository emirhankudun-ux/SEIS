...
| Include | `content/development/seis-model-scaling-hardware-profile.json`, `content/development/seis-20b-model-card-template.json`, `content/development/seis-20b-dataset-card-template.json`, `reports/seis-model-scaling/20b-16gb-memory-benchmark.json`, `docs/ai/seis-model-scaling.md`, `docs/ai/seis-ai-core.md`, `docs/ai/model-router.md`, `packages/seis-ai/src/lib/plugin-integration.mjs`, `packages/seis-ai/src/agent/tools.mjs`, `scripts/check-seis-model-scaling-hardware-profile.mjs`, `scripts/inspect-seis-model-local-hardware.mjs`, `package.json`, Command Center model-scaling UI, `/home/seis/Documents/seis-20b-local-preflight.md` dry-run VFS export, `dist/qa/model-scaling/local-hardware-preflight.json` ignored optional output, RAM compatibility profiles, benchmark manifest contract/template, model/dataset card templates, memory-budget contract, quantization lanes, creation stages, candidate-only local runtimes, status/backlog/index updates. |
...
| Validation | `npm run check:seis-model-local-hardware-preflight`, `npm run check:seis-model-scaling-hardware-profile`, `npm run check:seis-ai-core-provider-registry`, `npm test --prefix packages/seis-ai`, `git diff --check`. |
