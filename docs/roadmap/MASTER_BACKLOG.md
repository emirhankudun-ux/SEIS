...
| `SEIS-BL-015` | P1 | `@seis-design` | Add visual QA gates on top of the validator-backed component inventory. | `npm run check:design-component-inventory`, `npm run check:video-hero-showcase`, and `docs/reviews/VIDEO_HERO_SHOWCASE_QA.md` validate current design contracts and release gaps. |
...
| `SEIS-BL-019` | P1 | Plugin interfaces | Keep `@seis`, `@seis-cloud`, `@seis-code`, `@seis-design`, and `@seis-data` visible as a read-only static interface with a selectable five-year development program. | `npm run check:plugin-interface-roadmap` and `npm run check:data-schema-registry` validate the static UI bindings, support data, evidence paths, 2026-2030 horizon, and lane commitments. |
...
| `SEIS-BL-019` | `seis/plugin-interface-suite` | `feat: add SEIS plugin interface suite` | Medium | No unless adding dependencies or live integrations | Keep schema validation passing and refresh manual browser QA evidence for lane tabs, year controls, and program rows. |
