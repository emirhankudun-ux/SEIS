...
Persisting that anchor inside the durable checkpoint envelope is intentionally a separate follow-up so the existing hardened checkpoint schema is not silently widened without its own migration and corruption tests.

## Verification

Focused contract: `test/maria-recovery-reconciliation.test.py`.

The contract was committed first. Hosted MARIA regression CI failed while `maria_runtime.recovery_reconciliation` was absent. The implementation then added the bounded anchor, explicit dispositions, verified-context checks, deterministic drift-field reporting, and the invariant that reconciliation never authorizes execution.
