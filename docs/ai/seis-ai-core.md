...
| Conversation Nexus | Repo dışı OS-private runtime; varsayılan kapalı MCP metadata | `content/development/seis-conversation-nexus.json`, `packages/shared-types/schemas/seis-conversation-session.schema.json`, `seis://ai/conversation-nexus.json`, `npm run check:seis-conversation-nexus` | Yalnız redakte görünür kullanıcı/asistan metni tutulur. Tool payload, thinking, signature ve attachment kalıcılaştırılmaz. Conversation araçları bulut ajan döngüsünde yoktur; MCP metadata erişimi açık yerel opt-in ister. Session/export dosyaları local-keyfile AES-256-GCM envelope olarak yazılır. | Keychain/DPAPI/libsecret veya hardware-backed key storage olmadan daha güçlü encrypted-at-rest iddiası kurma; ChatGPT/Codex/Claude/Qwen importlarını kullanıcı seçimi, dry-run, provenance/redaction review ve ayrı onay olmadan etkinleştirme. |
...
The protocol-level MCP contract now exposes 39 tools, 33 resources, and three
