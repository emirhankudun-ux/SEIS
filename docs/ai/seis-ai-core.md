...
| AI workforce assignment registry | Bound as source-backed native and web metadata-only workforce evidence | `content/development/ai-workforce-assignments.json`, `packages/seis_platform_swift/Sources/SeisPlatformKit/SeisAIWorkforceAssignmentSnapshot.swift`, `packages/seis-ai/src/model/core-runtime-snapshot.mjs`, `apps/seis-core/data/seis-ai-core-runtime-snapshot.json`, `apps/seis-core/script.js` | The projection exposes ten declared roles, launcher readiness evidence, route, duties, allowed outputs, denied actions, validation duty, approval gates, and the exact metadata-only truth boundary. It does not claim live model access, authentication, provider calls, credential reads, autonomous execution, or external mutation. | Keep the source projection fail-closed, preserve the single-writer handoff rule, reject launcher/boundary drift, and require human approval for mutation, deployment, SSH, paid/live provider tests, merge, and protected-branch writes. |
| AI workforce training control plane | Bound as source-backed native and web metadata-only training evidence | `content/development/seis-ai-workforce-training-plan.json`, `packages/seis_platform_swift/Sources/SeisPlatformKit/SeisAIWorkforceTrainingSnapshot.swift`, `packages/seis-ai/src/model/core-runtime-snapshot.mjs`, `apps/seis-core/data/seis-ai-core-runtime-snapshot.json`, `apps/seis-core/script.js` | The projection exposes ten trainer roles, seven local training loops, four deterministic seed targets, launcher evidence, quality and automation records, safety rules, and acceptance gates. It does not run training, call providers, read credentials, download datasets, perform cloud fine-tuning, publish models, or grant runtime authority. | Keep model commands as recorded evidence only; require the local validator and human approval before any live provider, dataset, benchmark, SSH, deployment, merge, or model-publication action. |
| Agent permission matrix | Bound as source-backed native and web metadata-only governance evidence | `content/development/seis-ai-core-agent-permission-matrix.json`, `packages/seis_platform_swift/Sources/SeisPlatformKit/SeisAgentGovernanceSnapshot.swift`, `packages/seis_platform_swift/Sources/SeisPlatformKit/SeisAICoreRuntimeSnapshotContract.swift`, `packages/seis_platform_swift/Tests/SeisPlatformKitTests/SeisAgentGovernanceSnapshotTests.swift`, `apps/seis-core/data/seis-ai-core-runtime-snapshot.json` | Five permission levels, two enabled safe levels, exact approval requirements, and seven forbidden actions are decoded fail-closed. The generated snapshot adds no authority. | Keep read-only and plan-only as the only enabled levels; require a separate security/recovery plan before forbidden actions and keep native/web projections in parity. |
| Sub-agent runtime evidence | Bound as source-backed native and web metadata-only fixture evidence | `content/development/seis-ai-core-subagent-runtime-fixtures.json`, `content/development/seis-ai-core-execution-ledger-fixture.json`, `packages/seis_platform_swift/Sources/SeisPlatformKit/SeisAISubagentRuntimeFixturesSnapshot.swift`, `packages/seis_platform_swift/Sources/SeisPlatformKit/SeisAICoreRuntimeSnapshotContract.swift`, `apps/seis-core/data/seis-ai-core-runtime-snapshot.json` | Seven fixture references and a nineteen-field append-only planned ledger are validated; sample execution remains cancelled, redacted, and mutation-free. No durable audit store or agent execution exists. | Keep the ledger evidence-only until redaction, cancellation, permission, approval, and persistence tests pass under an approved runtime boundary. |
...
| Local model mode | Opt-in adapter with read-only preflight | `packages/seis_platform_swift/Sources/SeisPlatformKit/SeisAILocalLoopbackProviderAdapter.swift`, `packages/seis_platform_swift/Tests/SeisPlatformKitTests/SeisAIRuntimeTests.swift`, `packages/seis_platform_swift/Sources/SeisAppleNativeShell/Views/SeisAICoreLocalDemoView.swift` | The native surface can perform only a loopback `GET /api/tags` preflight and reports whether the configured model is listed. In the current environment the Ollama service responded successfully but exposed zero models; no prompt or model generation was performed. The default runtime remains Local Demo-only. | Enable model execution only with `approvedLocalLoopback`, an explicit local-only route, a loopback HTTP endpoint, a configured model identifier, and a human approval boundary. |
...
| Read-only router runtime | Bound as metadata-only provider-neutral runtime contract | `content/development/seis-ai-core-read-only-router-runtime.json`, `packages/seis-ai/src/model/read-only-router.mjs`, `packages/seis_platform_swift/Sources/SeisPlatformKit/SeisAICoreRuntimeSnapshotContract.swift`, `packages/seis_platform_swift/Tests/SeisPlatformKitTests/SeisAICoreRuntimeSnapshotContractTests.swift`, `apps/seis-core/script.js`, `apps/seis-core/data/seis-ai-core-runtime-snapshot.json` | Nine metadata inputs, five provider rules, five SEIS lanes, seven forbidden input classes, backend-only mediation evidence, plan-only permission-source status, and a false runtime/model-claim boundary are visible; no provider execution is enabled. | Keep route decisions deterministic, read-only, no-key, provider-neutral, backend-mediated, and fail-closed until a scoped backend adapter and approval evidence exist. |
| Model frontier escalation policy | Bound as metadata-only escalation gate | `content/development/seis-model-frontier-escalation-policy.json`, `packages/seis_platform_swift/Sources/SeisPlatformKit/SeisModelFrontierEscalationPolicySnapshot.swift`, `packages/seis_platform_swift/Tests/SeisPlatformKitTests/SeisModelFrontierEscalationPolicySnapshotTests.swift`, `npm run check:seis-model-frontier-escalation-policy` | Six stages keep only Local Demo allowed today; 20B, 70B, 150B, 512B, and future classes remain evidence-gated, approval-gated, and route-blocked. No weights, inference, benchmark, provider, ownership, or AGI evidence is implied. | Keep the no-skip ladder visible in native Command Center surfaces and require global evidence plus human approval before any frontier runtime or ownership claim. |
| AGI system source contract | Bound as source-backed AGI-inspired metadata | `content/development/seis-agi-system.json`, `packages/seis_platform_swift/Sources/SeisPlatformKit/SeisAGISystemSourceSnapshot.swift`, `packages/seis_platform_swift/Tests/SeisPlatformKitTests/SeisAGISystemSourceSnapshotTests.swift`, `npm run check:seis-agi-system` | Twenty priority domains, 150 taxonomy terms, 125 domain lanes, ten subsystems, five plugin capability lanes, three release milestones, and Apple-first memory/planning policy are visible; the source explicitly denies an autonomous general-intelligence claim. | Keep this source contract human-owned and metadata-only; do not turn its AGI-inspired roadmap, plugin lanes, or memory design into live autonomy, provider access, training, or model ownership. |
| Project intake safety contract | Bound as local-repo metadata-only intake policy | `content/development/seis-project-intake-contract.json`, `packages/seis_platform_swift/Sources/SeisPlatformKit/SeisProjectIntakeSnapshot.swift`, `packages/seis_platform_swift/Tests/SeisPlatformKitTests/SeisProjectIntakeSnapshotTests.swift`, `npm run check:seis-project-intake` | Read is allowed; write and shell require user approval; network is disabled by default; secret capture is forbidden. The contract does not prove a completed build or execute intake actions. | Keep native intake read-only by default and require explicit scope, approval, evidence, and rollback before any write, shell, network, or credential-related operation. |
| Connector capability registry | Bound as explicit-auth, registry-first capability metadata | `content/development/connector-capability-registry.json`, `packages/seis_platform_swift/Sources/SeisPlatformKit/SeisConnectorCapabilityRegistrySnapshot.swift`, `packages/seis_platform_swift/Tests/SeisPlatformKitTests/SeisConnectorCapabilityRegistrySnapshotTests.swift`, `npm run check:seis-agent-plugin-integration` | Twenty-one connectors, 50 skills, seven capability families, and five activation rules are source-backed; token commits, blanket OAuth, unreviewed remote mutation, broad scans, and unbounded browser runs remain blocked. | Use the registry for task-scoped capability selection only; authenticate, install, call, or write through a connector only after explicit target, scope, user approval, and rollback evidence. |
...
local training control-plane snapshot. The generated web artifact mirrors the
same ten trainer roles, seven training loops, and four deterministic seed-model
targets while the native panel consumes the typed source directly. Both expose
local quality and automation commands, launcher evidence, safety rules, and
acceptance gates.
...
Each generated route decision now carries its decision-integrity contract into
the web snapshot and typed native consumer. The contract requires explicit
provider state, redacted decision metadata, no prompt or credential material,
plan-only permission-source status (`verified` or `fail-closed`), and
`backendOnlyProvidersRequired: true`. The Command Center rejects the snapshot
if any scenario loses those fields; this is mediation evidence, not proof that
a provider call or autonomous sub-agent execution occurred.
...
The panel also reads `content/development/seis-technology-stack.json`. It
exposes 60 real source languages, seven ecosystem groups, 143 technologies, and
six requested core technologies. GitHub language panels remain separate from
...
twelve capability lanes, and 117 polyglot language surfaces. These are
