...
| SEIS demo 3D hero and AI Core constellation | Needs browser revalidation | The current branch statically aligns the demo with 35 MCP tools and 31 MCP resources; `npm run check:desktop-os` passes. A Chrome/Chromium executable is not available in this environment, so the browser interaction evidence has not been refreshed. |
...
- SEIS Code: Monaco ready, fallback editor hidden, terminal ready, 8 menus,
  5 activity views, 4 bottom panels, measured clickable-response coverage above
...
- Standalone SEIS demo: the current static contract exposes 35 MCP tools and
  31 MCP resources. Browser interaction revalidation remains pending until a
  Chrome/Chromium executable is available through `CHROME_PATH`.
