...
| `@seis-design` | Validator-backed inventory and repeatable browser-smoked showcase | `docs/design-system/seis-design-foundation.md`, `docs/design-system/component-inventory.md`, `content/development/seis-design-component-inventory.json`, `docs/product/video-hero-showcase.md`, `docs/reviews/VIDEO_HERO_SHOWCASE_QA.md`, `npm run check:design-component-inventory`, `npm run check:video-hero-showcase`, `npm run check:video-hero-browser-smoke` | No committed browser visual regression screenshots or performance budget. | Attach generated Video Hero screenshots to PR evidence and add visual QA evidence for current components. |
| Mythic Gacha | Playable static foundation with SEIS Code export bridge and repeatable product smoke | `apps/web/mythic-gacha.html`, `apps/web/mythic-gacha.js`, `apps/web/mythic-gacha.css`, `docs/product/mythic-gacha.md`, `npm run check:mythic-gacha`, `npm run check:product-experience-browser-smoke` | No committed visual regression baseline and no per-card artwork provenance review. | Keep draw/export smoke passing and add asset provenance review. |
...
| Plugin Interface Suite | Validator-backed static interface with interactive year program, H1/H2 cadence, and coverage metrics | `apps/web/index.html`, `content/development/seis-plugin-interface-roadmap.json`, `content/development/plugin-skill-capability-map.json`, `content/lab/cinematic-engine.json`, `content/lab/quality-console.json`, `docs/product/plugin-interface-suite.md`, `docs/reviews/PLUGIN_INTERFACE_SUITE_QA.md`, `scripts/check-plugin-interface-roadmap.mjs` | `/favicon.ico` fallback request still returns 404 in local browser QA. | Keep validator passing, verify year/cadence/coverage controls in browser QA, and clean favicon fallback before release readiness. |
