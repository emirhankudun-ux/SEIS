...
- 60 physical repository plugin sources, including 50 user-authorized local
  plugin packages owned by the SEIS Command Center under
  `plugins/seis-core`;
- 4940 deterministic catalog-only capability slots generated from the SEIS
...
The 50 app-owned personal packages currently share the internal Command Center
release train `0.00000001` (`0.0.10` strict semver). The release policy is recorded
...
packages owned by the SEIS Command Center under `plugins/seis-core`.
...
inspection plan, but a run, write, network, or secret action returns an
