...
63 app-owned `plugins/seis-core/*` source packages, canonical
...
- Keep the 63 app-owned packages on one gradual shared release ladder; the current release is `0.000000013`, large-code changes advance exactly one micro/revision step, annual updates advance one major step, and the supported range ends at `45.0000` without bulk jumps.
