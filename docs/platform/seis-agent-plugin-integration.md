...
- release train: `content/development/seis-core-plugin-release-train.json` (`0.000000014` / `0.0.14`)
- registry projection: `content/development/seis-ai-core-plugin-registry.json`
- current app-owned source count: 64
- public audience: everyone; source license: MIT
- public marketplace: `.agents/plugins/marketplace.json` (`seis-repo`)
- public marketplace entries: 370 total — 1 canonical SEIS-Agent, 5 migrated root packages, 64 app packages, and 300 objective-derived topic packages
...
64 app-owned `plugins/seis-core/*` source packages, canonical
...
- Keep the 64 app-owned packages on one gradual shared release ladder; the current release is `0.000000014`, large-code changes advance exactly one micro/revision step, annual updates advance one major step, and the supported range ends at `45.0000` without bulk jumps.
