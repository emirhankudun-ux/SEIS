...
This is an optional public SEIS Repo selection bundle that groups exactly 15 retained topic source capabilities. It is not a bulk installer and does not claim live provider, deployment, signing, or external write access.

## Components

- `.codex-plugin/plugin.json` defines the public bundle card.
- `.mcp.json` exposes a local read-only bundle MCP server.
- `assets/bundle-profile.json` records the deterministic 15-member map.
- `skills/seis-topic-bundle-26/SKILL.md` documents bounded selection and planning workflows.
