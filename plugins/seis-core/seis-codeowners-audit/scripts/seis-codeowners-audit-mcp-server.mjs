...
function status() {
  return {
    plugin: "seis-codeowners-audit",
    status: fs.existsSync(path.join(pluginRoot, "skills", "seis-codeowners-audit", "SKILL.md")) ? "ready" : "partial",
...
function audit(input) {
  ...
  const candidates = filesUnder(root).filter((file) => path.basename(file).toLowerCase() === "codeowners");
  const findings = [];
  const reports = [];
  for (const file of candidates) {
    ...
    for (const line of content.split(/\r?\n/)) {
      ...
      if (parts.length < 2) findings.push({ severity: "error", code: "ownerless-rule", file, rule: ruleCount });
      else if (!owners.some((owner) => owner.startsWith("@") || owner.includes("@"))) findings.push({ severity: "warning", code: "unverified-owner-token", file, rule: ruleCount });
    }
    reports.push({ file, ruleCount });
  }
  return {
    ...
    reports,
    findings,
    limitations: ["This checks local syntax only.", "It does not verify GitHub accounts, branch protection or external ownership."]
  };
}
const tools = [
  { name: "seis_codeowners_audit_status", description: "Report local CODEOWNERS audit readiness.", inputSchema: { type: "object", properties: {} } },
  { name: "seis_codeowners_audit", description: "Audit bounded local CODEOWNERS files.", inputSchema: { type: "object", properties: { path: { type: "string" } } } }
];
...
function handle(message) {
  ...
  if (message.method === "initialize") send({ jsonrpc: "2.0", id: message.id, result: { protocolVersion: "2024-11-05", capabilities: { tools: { listChanged: false } }, serverInfo: { name: "seis-codeowners-audit", version: "0.1.0" } } });
  else if (message.method === "tools/list") send({ jsonrpc: "2.0", id: message.id, result: { tools } });
  else if (message.method === "tools/call") {
    const name = message.params?.name;
    const args = message.params?.arguments || {};
    const result = name === "seis_codeowners_audit_status" ? status() : name === "seis_codeowners_audit" ? audit(args.path) : null;
