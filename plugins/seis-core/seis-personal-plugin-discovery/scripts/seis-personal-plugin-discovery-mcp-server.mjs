...
function status() {
  return {
    plugin: "seis-personal-plugin-discovery",
    status: fs.existsSync(path.join(pluginRoot, "skills", "seis-personal-plugin-discovery", "SKILL.md")) ? "ready" : "partial",
...
function discover(input) {
  ...
  let stat;
  try { stat = fs.lstatSync(root); } catch { return { state: "not-verified", ok: false, mode: "metadata-only-local-read-only", reason: "directory-not-found" }; }
  ...
  let entries = [];
  try { entries = fs.readdirSync(root, { withFileTypes: true }); } catch { return { state: "attention", ok: false, mode: "metadata-only-local-read-only", reason: "directory-unreadable" }; }
  const findings = [];
  const candidates = [];
  ...
  return {
    ...
    candidates,
    findings,
    limitations: ["Only direct children of the explicitly selected directory are inspected.", "Candidate code, assets, symlink targets, cloud folders, and device-wide paths are not scanned.", "Classification is metadata-derived and requires ownership and rights review before import."]
  };
}
const tools = [
  { name: "seis_personal_plugin_discovery_status", description: "Report metadata-only discovery readiness.", inputSchema: { type: "object", properties: {} } },
  { name: "seis_personal_plugin_discovery", description: "Discover plugin metadata under an explicitly selected directory.", inputSchema: { type: "object", properties: { path: { type: "string" } }, required: ["path"] } }
];
...
function handle(message) {
  ...
  if (message.method === "initialize") send({ jsonrpc: "2.0", id: message.id, result: { protocolVersion: "2024-11-05", capabilities: { tools: { listChanged: false } }, serverInfo: { name: "seis-personal-plugin-discovery", version: "0.1.0" } } });
  else if (message.method === "tools/list") send({ jsonrpc: "2.0", id: message.id, result: { tools } });
  else if (message.method === "tools/call") {
    const name = message.params?.name;
    const args = message.params?.arguments || {};
    const result = name === "seis_personal_plugin_discovery_status" ? status() : name === "seis_personal_plugin_discovery" ? discover(args.path) : null;
