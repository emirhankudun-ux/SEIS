...
function status() {
  return {
    plugin: "seis-migration-guide-check",
    status: fs.existsSync(path.join(pluginRoot, "skills", "seis-migration-guide-check", "SKILL.md")) ? "ready" : "partial",
...
function audit(input) {
  ...
  const candidates = filesUnder(root).filter((file) => /\.(md|markdown|mdx|txt)$/i.test(file) && /migration|upgrade|compatib|rollback|release/i.test(file));
  const required = [
    ["migration", /\bmigrat|upgrade|breaking change/i],
    ["compatibility", /\bcompatib|supported version|deprecat/i],
    ["rollback", /\brollback|revert|downgrade|recovery/i]
  ];
  const findings = [];
  const reports = [];
  for (const file of candidates) {
    ...
    const missing = required.filter(([, pattern]) => !pattern.test(content)).map(([id]) => id);
    if (missing.length) findings.push({ severity: "warning", code: "missing-migration-guidance", file, missing });
    reports.push({ file, missing });
  }
  return {
    ...
    reports,
    findings,
    limitations: ["Text presence is a structural signal, not proof that guidance is correct.", "No migration, rollback or release action is executed."]
  };
}
const tools = [
  { name: "seis_migration_guide_check_status", description: "Report migration guide check readiness.", inputSchema: { type: "object", properties: {} } },
  { name: "seis_migration_guide_check", description: "Check bounded local migration and rollback guidance.", inputSchema: { type: "object", properties: { path: { type: "string" } } } }
];
...
function handle(message) {
  ...
  if (message.method === "initialize") send({ jsonrpc: "2.0", id: message.id, result: { protocolVersion: "2024-11-05", capabilities: { tools: { listChanged: false } }, serverInfo: { name: "seis-migration-guide-check", version: "0.1.0" } } });
  else if (message.method === "tools/list") send({ jsonrpc: "2.0", id: message.id, result: { tools } });
  else if (message.method === "tools/call") {
    const name = message.params?.name;
    const args = message.params?.arguments || {};
    const result = name === "seis_migration_guide_check_status" ? status() : name === "seis_migration_guide_check" ? audit(args.path) : null;
