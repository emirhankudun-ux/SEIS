...
function recordsFrom(data) {
  ...
  for (const key of ["pullRequests", "pulls", "prs", "reviews", "records"]) {
...
function status() {
  return {
    plugin: "seis-pr-cycle-time",
    status: fs.existsSync(path.join(pluginRoot, "skills", "seis-pr-cycle-time", "SKILL.md")) ? "ready" : "partial",
    mode: "local-read-only",
    network: "disabled-by-design",
    writes: "disabled-by-design",
    source: "recorded-local-pull-request-evidence"
  };
}
function audit(input) {
  ...
  const candidates = filesUnder(root).filter((file) => /\.json$/i.test(file) && /pr|pull|review|cycle/i.test(file));
  const findings = [];
  const reports = [];
  const durations = [];
  for (const file of candidates) {
    ...
    for (const [index, record] of recordsFrom(data).entries()) {
      const openedAt = timeFrom(record, ["openedAt", "createdAt", "startedAt"]);
      const mergedAt = timeFrom(record, ["mergedAt", "closedAt", "completedAt"]);
      const explicitHours = Number(valueFrom(record, ["cycleTimeHours", "durationHours"]));
      const cycleTimeHours = Number.isFinite(explicitHours) ? explicitHours : openedAt !== null && mergedAt !== null ? (mergedAt - openedAt) / 3600000 : null;
      const valid = cycleTimeHours !== null && cycleTimeHours >= 0;
      if (valid) durations.push(cycleTimeHours);
      if (openedAt !== null && mergedAt !== null && mergedAt < openedAt) findings.push({ severity: "error", code: "negative-cycle-time", file, record: index + 1 });
      if (!valid) findings.push({ severity: "warning", code: "missing-cycle-time-evidence", file, record: index + 1 });
      reports.push({ file, record: index + 1, cycleTimeHours: valid ? cycleTimeHours : null, state: String(valueFrom(record, ["state", "status"]) ?? "unknown") });
    }
  }
  return {
    ...
    metrics: { samples: durations.length, averageHours: average(durations), medianHours: median(durations), p90Hours: percentile(durations, 0.9) },
    reports,
    findings,
    limitations: ["Only recorded PR JSON evidence is analyzed.", "Open PRs without a completion timestamp are reported as missing evidence.", "No current GitHub or team-wide claim is made."]
  };
}
const tools = [
  { name: "seis_pr_cycle_time_status", description: "Report PR cycle-time analysis readiness.", inputSchema: { type: "object", properties: {} } },
  { name: "seis_pr_cycle_time", description: "Measure recorded pull-request cycle times.", inputSchema: { type: "object", properties: { path: { type: "string" } } } }
];
...
function handle(message) {
  ...
  if (message.method === "initialize") send({ jsonrpc: "2.0", id: message.id, result: { protocolVersion: "2024-11-05", capabilities: { tools: { listChanged: false } }, serverInfo: { name: "seis-pr-cycle-time", version: "0.1.0" } } });
  else if (message.method === "tools/list") send({ jsonrpc: "2.0", id: message.id, result: { tools } });
  else if (message.method === "tools/call") {
    const name = message.params?.name;
    const args = message.params?.arguments || {};
    const result = name === "seis_pr_cycle_time_status" ? status() : name === "seis_pr_cycle_time" ? audit(args.path) : null;
