...
test("keeps the requested 30-step bootstrap and five 100-step waves evidence-led and public-only", () => {
  ...
  assert.equal(cadence.waves[2].completedSteps, 99);
  assert.deepEqual(cadence.waves[2].inProgressSteps, [100]);
  ...
  assert.equal(cadence.waves[2].currentEvidencePath, "content/development/seis-public-plugin-wave-4-program.json");
  assert.equal(cadence.waves[3].status, "planned-gated");
  assert.equal(cadence.waves[3].plannedProgramPath, "content/development/seis-public-plugin-wave-4-program.json");
  assert.equal(cadence.waves[3].selectedCapability, "seis-swift-package-topology");
  assert.equal(cadence.waves[3].activationApproved, false);
