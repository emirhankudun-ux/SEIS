...
test("keeps the requested 30-step bootstrap and five 100-step waves evidence-led and public-only", () => {
  ...
  assert.equal(cadence.waves[2].completedSteps, 97);
  assert.deepEqual(cadence.waves[2].inProgressSteps, [98]);
  ...
  assert.equal(cadence.waves[2].currentEvidencePath, "content/development/seis-public-plugin-wave-3-repository-local-handoff.json");
