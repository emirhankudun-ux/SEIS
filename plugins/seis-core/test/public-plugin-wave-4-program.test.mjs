...
test("records the first bounded Wave 4 topology integration checkpoint", () => {
  ...
  assert.ok(program.steps.every((step, index) => step.number === index + 1 && step.status === (index < 90 ? "completed" : index === 90 ? "in-progress" : "planned")));
  ...
  assert.deepEqual(program.progress.inProgressStepNumbers, [91]);
  assert.equal(program.progress.completedStepCount, 90);
  assert.equal(program.progress.completedRoundCount, 4);
  assert.equal(program.progress.plannedStepCount, 9);
