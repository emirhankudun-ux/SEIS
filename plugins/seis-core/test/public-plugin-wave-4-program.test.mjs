...
test("defines a separate unactivated 100-step Wave 4 topology plan", () => {
  ...
  assert.equal(program.status, "planned-gated");
  ...
  assert.ok(program.steps.every((step, index) => step.number === index + 1 && step.status === "planned"));
  assert.equal(program.scope.selectedCapability, "seis-swift-package-topology");
  assert.equal(program.activationGate.status, "not-approved");
