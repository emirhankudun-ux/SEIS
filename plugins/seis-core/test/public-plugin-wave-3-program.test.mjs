...
test("keeps Wave 3 as an evidence-led public SEIS Repo implementation with one approved card", () => {
  ...
  assert.equal(program.progress.completedStepCount, 79);
  assert.equal(program.progress.plannedStepCount, 20);
  assert.deepEqual(program.progress.inProgressStepNumbers, [80]);
  assert.equal(program.progress.completedRoundCount, 3);
