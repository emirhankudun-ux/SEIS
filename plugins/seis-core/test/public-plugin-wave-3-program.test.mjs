...
test("keeps Wave 3 as an evidence-led public SEIS Repo implementation with one approved card", () => {
  ...
  assert.equal(program.progress.completedStepCount, 91);
  assert.equal(program.progress.plannedStepCount, 8);
  assert.deepEqual(program.progress.inProgressStepNumbers, [92]);
