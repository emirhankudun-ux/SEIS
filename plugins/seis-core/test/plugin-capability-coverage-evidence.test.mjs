...
test("reconciles bounded public SEIS Repo capability coverage evidence", () => {
  ...
  assert.equal(typeof evidence.plugin.publicBundleId, "string");
  ...
  assert.equal(evidence.marketplace.publicCardCount, 34);
  assert.equal(evidence.marketplace.bundlePluginCount, 33);
  assert.equal(evidence.marketplace.applicationBundleMemberCount, APP_PLUGIN_EXPANSION_TARGET);
  ...
  assert.equal(evidence.audit.bundleApplicationMemberCount, APP_PLUGIN_EXPANSION_TARGET);
  ...
  assert.equal(evidence.audit.declaredCategoryCount, evidence.audit.reportedCategoryCount);
  assert.equal(evidence.audit.declaredCapabilityTokenKindCount, evidence.audit.reportedCapabilityTokenKindCount);
  assert.equal(evidence.audit.coverageOutputTruncated, false);
  assert.equal(evidence.outputBounds.aggregateOnly, true);
  assert.equal(evidence.outputBounds.maxReturnedCategoryKinds, 128);
  assert.equal(evidence.outputBounds.maxReturnedCapabilityTokenKinds, 256);
  assert.equal(evidence.outputBounds.maxReturnedFindings, 64);
  assert.equal(evidence.outputBounds.categoryCountsTruncated, false);
  assert.equal(evidence.outputBounds.capabilityTokenFrequenciesTruncated, false);
