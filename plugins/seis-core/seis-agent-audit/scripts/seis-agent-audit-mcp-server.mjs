...
function status() {
  return {
    plugin: "seis-agent-audit",
    status: fs.existsSync(path.join(pluginRoot, "skills", "seis-agent-audit", "SKILL.md")) ? "ready" : "partial",
...
function recordsFrom(data) {
  ...
  for (const key of ["agents", "records", "entries", "contracts"]) if (Array.isArray(data?.[key])) return data[key].filter((value) => value && typeof value === "object");
...
function audit(input) {
  ...
  const candidates = filesUnder(root).filter((file) => /\.json$/i.test(file) && /agent/i.test(file));
  const findings = [];
  const reports = [];
  for (const file of candidates) {
    ...
    for (const [index, record] of recordsFrom(data).entries()) {
      const label = String(record.id || record.name || "agent");
      const permissions = record.permissions || record.allowedTools || record.allowedInputs;
      const evidence = record.evidence || record.validation || record.expectedOutputs;
      const cancellation = record.cancellation || record.cancel || record.stop;
      const auditEvents = record.auditEvents || record.audit || record.telemetry;
      const handoff = record.handoff || record.handoffContract;
      const missing = [];
      if (!permissions) missing.push("permissions");
      if (!evidence) missing.push("evidence");
      if (!cancellation) missing.push("cancellation");
      if (!auditEvents) missing.push("auditEvents");
      if (!handoff) missing.push("handoff");
      if (missing.length) findings.push({ severity: "warning", code: "missing-agent-governance", file, record: index + 1, agent: label, missing });
      reports.push({ file, record: index + 1, agent: label, missing });
    }
  }
  return {
    ...
    reports,
    findings,
    limitations: ["Only local JSON agent metadata is inspected.", "No agent, tool or permission is executed or changed."]
  };
}
const tools = [
  { name: "seis_agent_audit_status", description: "Report local agent audit readiness.", inputSchema: { type: "object", properties: {} } },
  { name: "seis_agent_audit", description: "Audit bounded local agent evidence and handoff metadata.", inputSchema: { type: "object", properties: { path: { type: "string" } } } }
];
...
function handle(message) {
  ...
  if (message.method === "initialize") send({ jsonrpc: "2.0", id: message.id, result: { protocolVersion: "2024-11-05", capabilities: { tools: { listChanged: false } }, serverInfo: { name: "seis-agent-audit", version: "0.1.0" } } });
  else if (message.method === "tools/list") send({ jsonrpc: "2.0", id: message.id, result: { tools } });
  else if (message.method === "tools/call") {
    const name = message.params?.name;
    const args = message.params?.arguments || {};
    const result = name === "seis_agent_audit_status" ? status() : name === "seis_agent_audit" ? audit(args.path) : null;
