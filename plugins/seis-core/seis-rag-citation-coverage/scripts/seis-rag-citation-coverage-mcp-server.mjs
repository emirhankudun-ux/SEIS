...
function status() {
  return {
    plugin: "seis-rag-citation-coverage",
    status: fs.existsSync(path.join(pluginRoot, "skills", "seis-rag-citation-coverage", "SKILL.md")) ? "ready" : "partial",
    mode: "local-read-only",
    retrievalCalls: false,
    documentContent: "not-emitted",
...
function recordsFrom(data) {
  ...
  for (const key of ["results", "records", "responses", "items", "outputs"]) if (Array.isArray(data?.[key])) return data[key].filter((value) => value && typeof value === "object");
...
function audit(input) {
  ...
  const candidates = filesUnder(root).filter((file) => /\.json$/i.test(file) && /rag|retriev|citation|search|knowledge/i.test(file));
  const findings = [];
  const reports = [];
  for (const file of candidates) {
    ...
    for (const [index, record] of records.entries()) {
      const citations = record.citations || record.sources || record.references || record.sourceIds || [];
      const citationCount = Array.isArray(citations) ? citations.length : citations ? 1 : 0;
      if (citationCount === 0) findings.push({ severity: "warning", code: "missing-citation", file, record: index + 1 });
      reports.push({ file, record: index + 1, citationCount });
    }
  }
  const total = reports.length;
  const cited = reports.filter((report) => report.citationCount > 0).length;
  return {
    ...
    citedRecords: cited,
    citationCoveragePercent: total ? Math.round((cited / total) * 100) : null,
    reports,
    findings,
    limitations: ["Only recorded JSON citation fields are counted.", "No retrieval, embedding, reranking or document content is executed or emitted."]
  };
}
const tools = [
  { name: "seis_rag_citation_coverage_status", description: "Report RAG citation coverage readiness.", inputSchema: { type: "object", properties: {} } },
  { name: "seis_rag_citation_coverage", description: "Measure citations in bounded local retrieval records.", inputSchema: { type: "object", properties: { path: { type: "string" } } } }
];
...
function handle(message) {
  ...
  if (message.method === "initialize") send({ jsonrpc: "2.0", id: message.id, result: { protocolVersion: "2024-11-05", capabilities: { tools: { listChanged: false } }, serverInfo: { name: "seis-rag-citation-coverage", version: "0.1.0" } } });
  else if (message.method === "tools/list") send({ jsonrpc: "2.0", id: message.id, result: { tools } });
  else if (message.method === "tools/call") {
    const name = message.params?.name;
    const args = message.params?.arguments || {};
    const result = name === "seis_rag_citation_coverage_status" ? status() : name === "seis_rag_citation_coverage" ? audit(args.path) : null;
