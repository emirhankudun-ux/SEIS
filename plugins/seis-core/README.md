...
66 app-owned plugins used by the SEIS Command Center application. Everyone can
...
All 66 packages move together on the gradual release train:

```text
0.000000001 -> 0.00000001 -> 0.000000011 -> ... -> 1.0000 -> 1.0001 -> ... -> 45.0000
```

The current release is `0.000000016`, represented as strict package semver
`0.0.16`. A large, measured code change advances exactly one micro/revision
...
by the same 66 public repository manifests instead of a second static source of truth.
...
`seis-ai-agent@seis-repo`; the 66 app-owned packages are public app packages,
...
- public marketplace app section: one canonical orchestrator card plus 66 public app package cards
- complete `seis-repo` marketplace: 372 cards, including five migrated root cards and 300 objective-derived `plugins/seis-topics` packages
