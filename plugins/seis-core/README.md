...
62 app-owned plugins used by the SEIS Command Center application. Everyone can
...
All 62 packages move together on the gradual release train:
...
by the same 62 public repository manifests instead of a second static source of truth.
...
`seis-ai-agent@seis-repo`; the 62 app-owned packages are not personal plugins,
...
- public marketplace app section: one canonical orchestrator card plus 62 public app package cards
- complete `seis-repo` marketplace: 368 cards, including five migrated root cards and 300 objective-derived `plugins/seis-topics` packages
