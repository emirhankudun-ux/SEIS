...
- `plugins/seis-core/`: 70 app-owned public plugin packages inside the SEIS repository plugin boundary; current shared release is `0.00000002` (`0.0.20` in strict plugin manifests), including ten bounded read-only audit/report packages plus public marketplace-integrity, public-distribution, trusted-marketplace, public-install-state, public-install-evidence, public-runtime-status, MCP-permission-boundary, static focus-navigation, static UI-state-contract, and project-manifest validators.
...
- Plugins & Extensions: plugin family health, the generated 70-plugin app catalog, search, permission posture, update gates, activation policy, and the app-owned plugin source boundary.
...
- Inspect the 70 public app-owned plugin packages from `plugins/seis-core` through the source inventory and read-only AI Core registry projection.
- Search and inspect the same 70 packages from the Command Center catalog; status checks remain bounded to each plugin's local `--status` entrypoint.
