...
- `data/seis-ai-core-runtime-snapshot.json`: deterministic bridge from the provider registry, executable read-only router, public 9-lane/13-agent registry, source-backed 10-role AI workforce assignment registry, 10-role/7-loop/4-target AI workforce training control plane, unified SEIS-Agent plugin audit, five personal lanes, and local MCP smoke contract into SEIS Core and the typed Swift consumer. Its plugin MCP mesh also records one allowlisted local status probe per bundled server, with no credentials, network, live session, or mutation; the runtime panel renders each verified probe as redacted status metadata and rejects incomplete lifecycle evidence.
