...
- `plugins/seis-core/`: 62 app-owned local plugin packages inside the SEIS repository plugin boundary; current shared release is `0.000000013` (`0.0.13` in strict plugin manifests), including ten bounded read-only audit/report packages plus public marketplace-integrity and public-distribution validators.
...
- Plugins & Extensions: plugin family health, the generated 62-plugin app catalog, search, permission posture, update gates, activation policy, and the app-owned plugin source boundary.
...
- Inspect the 62 local app-owned plugin packages from `plugins/seis-core` through the source inventory and read-only AI Core registry projection.
- Search and inspect the same 62 packages from the Command Center catalog; status checks remain bounded to each plugin's local `--status` entrypoint.
