...
- `plugins/seis-core/`: 75 app-owned public source packages inside the SEIS repository plugin boundary; current shared release is `0.00000002` (`0.0.20` in strict plugin manifests). They remain source capabilities, are discoverable through six bounded application bundle cards, and include ten bounded read-only audit/report packages plus public marketplace-integrity, public-distribution, trusted-marketplace, public-install-state, public-install-evidence, public-runtime-status, MCP-permission-boundary, static focus-navigation, static UI-state-contract, project-manifest, and Wave 1 evidence-index validators.
...
- Plugins & Extensions: plugin family health, the generated 75-source app catalog, search, permission posture, update gates, activation policy, six application bundle cards, and the app-owned plugin source boundary.
...
- Inspect the 75 public app-owned source packages from `plugins/seis-core` through the source inventory and read-only AI Core registry projection.
- Search and inspect the same 75 source packages from the Command Center catalog; status checks remain bounded to each plugin's local `--status` entrypoint and marketplace discovery resolves through six reviewed application bundle cards.
...
- Inspect the gradual app plugin release train at `0.00000002`; large-code changes advance one micro step, annual/major updates advance one major step, and no bulk jump is allowed.
