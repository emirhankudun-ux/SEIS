...
- `data/seis-ai-core-runtime-snapshot.json`: deterministic bridge from the provider registry, executable read-only router, public 9-lane/13-agent registry, source-backed five-level AI agent permission matrix, seven-fixture sub-agent runtime evidence pack with append-only execution-ledger contract, 10-role AI workforce assignment registry, 10-role/7-loop/4-target AI workforce training control plane, unified SEIS-Agent plugin audit, five personal lanes, and local MCP smoke contract into SEIS Core and the typed Swift consumer. Each route decision also carries typed decision-integrity evidence for redaction, plan-only permission-source status, and backend-only provider mediation. Its plugin MCP mesh records one allowlisted local status probe per bundled server, with no credentials, network, live session, or mutation; the runtime panel renders each verified probe as redacted status metadata and rejects incomplete lifecycle evidence. Permission levels, route integrity, and runtime fixtures remain status-and-plan metadata and do not grant execution authority.
- `packages/seis_platform_swift/Sources/SeisPlatformKit/SeisAIInstalledCapabilityInventorySnapshot.swift`: typed native projection of the source-backed Big Tech MCP/skill and NVIDIA installed-integration inventories. It exposes counts and redacted statuses for 38 skills, 3 local tool profiles, 17 current-session MCP surfaces, and 11 NVIDIA integrations while keeping activation, credentials, network, runtime commands, and external mutation approval-gated.
- `packages/seis_platform_swift/Sources/SeisPlatformKit/SeisAIWorkforceAssignmentSnapshot.swift`: typed native projection of the ten-role workforce assignment registry, including exact launcher evidence, approval gates, truth boundary, launcher status, role duties, allowed outputs, denied actions, validation duty, and the Codex primary-writer policy.
- `packages/seis_platform_swift/Sources/SeisPlatformKit/SeisAIWorkforceTrainingSnapshot.swift`: typed native projection of the ten trainer roles, seven local training loops, four deterministic seed targets, launcher evidence, acceptance gates, and no-live-training boundary.
...
- `packages/seis_platform_swift/Sources/SeisAppleNativeShell/Views/SeisAICoreLocalDemoView.swift`: Apple-native Local Demo controls for all five personal lanes, all 13 managed agents, the ten source-backed workforce roles, and the local training control plane; every action is status-and-plan-only and refreshes the redacted evidence ledger.
...
The current projection contains six Core lanes, six bundled plugin sources, 25 repository skills, seven providers, MCP coverage of 37 tools / 30 resources / three prompts, six local plugin MCP entrypoints, 18 product modules, 18 data contracts, 12 design components, 13 managed agent roles, 10 source-backed workforce assignments, 7 training loops, and 4 deterministic seed targets. The 185 installed-enabled plugin value is a dated source audit, while the 300 helper-plugin value is only a catalogued universe. Neither value means every plugin, role, or route is authenticated, active for the current task, or permitted to mutate an external system.
