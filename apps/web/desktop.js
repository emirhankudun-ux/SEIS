...
const SEIS_SECOND_BRAIN_AGENT_REVIEW_QUEUE = {
  ...
  requiredArtifacts: [
    ...
    "/home/seis/SecondBrain/obsidian-explicit-selection-receipt.md",
    "/home/seis/SecondBrain/obsidian-preflight-approval-request.md",
    "/home/seis/SecondBrain/obsidian-safe-import-ui-dry-run.md",
    SEIS_SECOND_BRAIN_AGENT_REGISTRY.jsonPath,
    SEIS_SECOND_BRAIN_AGENT_REGISTRY.markdownPath
  ]
};
...
function renderSecondBrain() {
  ...
  return `<section class="app-main second-brain-app" data-second-brain-app>
    <div class="toolbar">
      ...
      <button type="button" data-action="second-brain-build-agent-review-queue">Build Agent Review Queue</button>
    ...
    <section class="subagent-panel second-brain-ai-index" data-second-brain-ai-index>
      ...
      <section class="subagent-panel" data-second-brain-agent-review-assignment>
        ...
        <table class="data-table" data-second-brain-agent-review-ledger>
          <thead><tr><th>Recorded</th><th>Agent</th><th>Status</th><th>Brief</th><th>Local context</th></tr></thead>
          <tbody>${data.agentReviewAssignments.map((assignment) => `<tr><td>${escapeHtml(new Date(assignment.recordedAt).toLocaleTimeString())}</td><td>${escapeHtml(assignment.agent.name)}</td><td>${escapeHtml(assignment.status)}</td><td>${escapeHtml((assignment.humanReviewBrief?.text || "No brief recorded.").slice(0, 120))}</td><td>${escapeHtml(assignment.contextProfiles.map((profile) => profile.lane).join(", ") || "No mapped lane")}</td></tr>`).join("") || "<tr><td colspan=\"5\">No plan-only assignments recorded.</td></tr>"}</tbody>
        </table>
      </section>
...
## Plan-Only Agent Queue

${agentQueue.join("\n")}

## Human Review Requirements

- Confirm provenance, no-secret status, and mock/real/planned labels before using any referenced artifact.
- Approve each scoped task before an agent performs a write, connector call, provider call, SSH action, GitHub action, deployment, or private-vault operation.
- Keep autonomous execution blocked until an explicit human approval record exists.
`;
}
...
function recordSecondBrainAgentReviewAssignment(button) {
  ...
  const reviewBrief = root?.querySelector("[data-second-brain-agent-review-brief]")?.value?.trim() || "";
  if (!reviewBrief) {
    toast("SEIS Second Brain", "Add a human-authored review brief before recording the plan-only assignment.");
    return;
  }
  if (reviewBrief.length > SEIS_SECOND_BRAIN_AGENT_REVIEW_ASSIGNMENT.maxBriefCharacters) {
