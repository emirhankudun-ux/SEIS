...
const initialize=()=>({protocolVersion:'2025-06-18',capabilities:{tools:{}},serverInfo:{name:'fixture',version:'1.0.0'}});
...
test('discovers tools only after initialize handshake', async()=>{
  const calls=[];
  const adapter=createMcpAdapter({protocolVersion:'2025-06-18',providerId:'mcp',authorizeTool:()=>true,transport:transport(async req=>{
    calls.push(req.method);
    if(req.method==='initialize') return initialize();
    if(req.method==='tools/list') return {tools:[tool('repo.read')]};
  })});
...
test('abort signal cancels tool call', async()=>{
  ...
  const adapter=createMcpAdapter({protocolVersion:'2025-06-18',providerId:'mcp',authorizeTool:()=>true,transport:async req=>{
    if(req.method==='initialize') return initialize();
    if(req.method==='tools/list') return {tools:[tool('slow')]};
