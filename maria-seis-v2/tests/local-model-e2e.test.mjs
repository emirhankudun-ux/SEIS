...
test('local model can complete the full verified live inference contract', async()=>{
  const fetchImpl=async(url,options={})=>{
    ...
    if(String(url).endsWith('/v1/chat/completions')) return response({
      id:'chatcmpl-e2e-1',model:'qwen-local',choices:[{message:{role:'assistant',content:'SEIS hazır.'}}]
    });
  ...
  assert.equal(result.verification.scope,'model-response-transport');
