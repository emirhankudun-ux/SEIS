...
function renderStatic() {
  // These catalogs are authored application data. User commands always use textContent.
...
function activity(agent,title,body,progress=100,result='Harici işlem yok',tone='amber') {
  return {agent,title,body,progress,result,tone};
}
async function submitCommand(command) {
  ...
  store.set({busy:true,systemState:'UNDERSTANDING',message:{headline:'İş akışına bakalım.',body:command},
    activity:activity('Intent Engine','Komut çözümleniyor',command,15)});
  try {
    ...
    if (outcome.status === 'simulated') {
      store.set({systemState:'SIMULATED',message:{headline:'Simülasyon tamamlandı.',body:outcome.verification.summary},
        activity:activity('Contract verifier','Simülasyon sözleşmesi geçti',outcome.verification.evidence.join(' · '),100,'Gerçek görev doğrulanmadı')});
    } else if (outcome.status === 'approval') {
      $('#approvalCard').classList.remove('hidden'); $('#approvalText').textContent = outcome.reason;
      store.set({systemState:'APPROVAL REQUIRED',message:{headline:'İşlem yapılmadı.',body:'Bu komut açık yetkilendirme gerektiriyor. Demo harici işlem yapamaz.'},
        activity:activity('Permission Engine','Yetkilendirme gerekiyor',outcome.reason,0,'Engellendi','red')});
    } else {
      ...
      store.set({systemState:outcome.status.toUpperCase(),message:{headline,body:outcome.reason ?? 'Başarı iddiası yok; harici işlem yapılmadı.'},
        activity:activity('MARIA',headline,outcome.reason ?? 'Doğrulama başarısız.',0,'Tamamlanmadı',outcome.status==='cancelled'?'amber':'red')});
    }
  } catch {
    store.set({systemState:'ERROR',message:{headline:'Bir hata oluştu.',body:'İşlem doğrulanmadı. Yeniden deneyebilirsin.'},
      activity:activity('MARIA','Beklenmeyen hata','Hassas hata ayrıntıları ekrana yazılmadı.',0,'Doğrulanmadı','red')});
  } finally {
    controller = null; store.set({busy:false}); $('#commandInput').focus();
  }
}
function send() {
  if (store.get().busy) {controller?.abort();return;}
  const input = $('#commandInput'); const text=input.value; input.value=''; void submitCommand(text);
}
function changeProject(id) {
  if (store.get().busy || !projects.some(project=>project.id===id)) return;
  $('#approvalCard').classList.add('hidden');
  store.set({projectId:id,systemState:'READY',message:null,activity:activity('MARIA','Proje bağlamı değişti','Yalnızca oturum bağlamı değişti; proje dosyalarına erişilmedi.',0)});
}
function explainUnavailable(name) {
  store.set({systemState:'UNAVAILABLE',message:{headline:`${name} bağlı değil.`,body:'Bu alpha sürümünde yalnızca iş akışı simülasyonu var. Kayıt veya harici işlem başlatılmadı.'},
    activity:activity('Capability registry','Adaptör gerekiyor',`${name} için gerçek bağlantı henüz yok.`,0)});
}
function closeProjectMenu() { $('#projectMenu').classList.add('hidden'); $('#projectSwitch').setAttribute('aria-expanded','false'); }
function openSettings() {
  previousFocus=document.activeElement; closeProjectMenu();
  $('#settingsSheet').classList.remove('hidden'); $('.app-shell').inert=true; $('#closeSettings').focus();
}
function closeSettings() {
  $('#settingsSheet').classList.add('hidden'); $('.app-shell').inert=false;
...
$('#projectSwitch').addEventListener('click',()=>{
  const menu=$('#projectMenu');const show=menu.classList.contains('hidden');
  const rect=$('#projectSwitch').getBoundingClientRect();menu.style.top=`${rect.bottom+8}px`;menu.style.left=`${Math.max(12,rect.right-190)}px`;
  menu.classList.toggle('hidden',!show);$('#projectSwitch').setAttribute('aria-expanded',String(show));if(show) menu.querySelector('button').focus();
});
...
$('#settingsList').addEventListener('click',event=>{
  const button=event.target.closest('[data-setting]');if(!button || button.disabled) return;
  const definition=settings.find(item=>item.id===button.dataset.setting);if(!definition?.editable)return;
  const current=store.get();store.set({settings:{...current.settings,[definition.id]:!current.settings[definition.id]}});
});
$('#approvalCard').addEventListener('click',()=>{
  $('#approvalCard').classList.add('hidden');store.set({systemState:'READY',message:{headline:'İstek kapatıldı.',body:'Onay verilmedi; harici işlem yapılmadı.'},activity:activity('Permission Engine','İstek kapatıldı','Kapatmak, yürütme onayı değildir.',0)});
});
document.addEventListener('click',event=>{if(!event.target.closest('#projectMenu, #projectSwitch'))closeProjectMenu();});
document.addEventListener('keydown',event=>{
  const dialogOpen=!$('#settingsSheet').classList.contains('hidden');
  if(event.key==='Escape'){if(dialogOpen)closeSettings();else closeProjectMenu();}
  if(dialogOpen && event.key==='Tab'){
    const focusable=[...$('#settingsSheet').querySelectorAll('button:not(:disabled), input:not(:disabled), a[href]')];
    const first=focusable[0],last=focusable.at(-1);
    if(event.shiftKey && document.activeElement===first){event.preventDefault();last.focus();}
    else if(!event.shiftKey && document.activeElement===last){event.preventDefault();first.focus();}
  }
  if(!dialogOpen && (event.metaKey||event.ctrlKey) && event.key.toLowerCase()==='k'){event.preventDefault();$('#commandInput').focus();}
});
