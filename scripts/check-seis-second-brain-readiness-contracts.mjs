...
function validatePublicDemoArtifacts(report, manifest) {
  ...
  const browserSmokeMissing = report.blockers.includes("current-browser-smoke-evidence-missing");
  ensure(Boolean(browserSmokeItem), "public demo evidence manifest must include current-browser-smoke item.");
  if (browserSmokeMissing) {
    ensure(
      browserSmokeItem?.status === "missing-current-evidence",
      "public demo evidence manifest must keep browser-smoke status honest when current evidence is missing."
    );
    ensure(
      manifest.summary?.missingCurrentEvidence >= 1,
      "public demo evidence manifest must count missing current browser-smoke evidence."
    );
  } else {
    ensure(browserSmokeItem?.status === "passed", "public demo evidence manifest must mark browser-smoke passed only after current evidence exists.");
    ensure(manifest.summary?.missingCurrentEvidence === 0, "public demo evidence manifest should have no missing evidence after current browser-smoke passes.");
  }
