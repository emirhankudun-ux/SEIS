...
function buildRecord() {
  ...
  const marketplaceEntry = list(marketplace.plugins).find((entry) => entry?.name === PLUGIN_CAPABILITY_COVERAGE_ID);
  ...
  const record = {
    ...
    plugin: {
      ...
      publicMarketplace: marketplaceEntry?.source?.path === `./plugins/seis-core/${PLUGIN_CAPABILITY_COVERAGE_ID}`,
    },
    marketplace: {
    ...
    rollback: {
      strategy: "revert",
      scope: "Revert the local package, one SEIS Repo card, and generated evidence record; no registry source, external system, or release state is mutated by the plugin runtime.",
