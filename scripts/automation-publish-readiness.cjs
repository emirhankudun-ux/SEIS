...
print(`- expected branch (${state.expectedBranch || "main"}): ${state.isExpectedBranch ? "yes" : "no"}`);
...
print(`- expected upstream (${state.expectedUpstream || "origin/main"}): ${state.hasUpstream && state.isExpectedUpstream ? "yes" : "no"}`);
