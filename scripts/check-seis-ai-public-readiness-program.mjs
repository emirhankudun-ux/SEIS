...
ensure(retrievalEvaluationFixtures?.status === "fixtures-defined-not-run", "retrieval evaluation fixtures must remain not-run");
ensure(retrievalEvaluationFixtures?.approvedToday?.evaluationRun === false, "retrieval evaluation fixtures must not approve an evaluation run");
ensure(retrievalEvaluationFixtures?.approvedToday?.retrievalIndexQuery === false, "retrieval evaluation fixtures must not query an index");
...
ensure(retrievalCitationScorerDryRun?.status === "citation-scorer-dry-run-passed-not-measured", "retrieval citation scorer dry-run must pass without measured scoring");
ensure(retrievalCitationScorerDryRun?.approvedToday?.liveCitationScoring === false, "retrieval citation scorer dry-run must not approve live citation scoring");
ensure(retrievalCitationScorerDryRun?.approvedToday?.answerGeneration === false, "retrieval citation scorer dry-run must not generate answers");
ensure(retrievalCitationScorerDryRun?.approvedToday?.retrievalIndexQuery === false, "retrieval citation scorer dry-run must not query an index");
