...
async function smokeSecondBrain(client, baseUrl) {
  ...
  const initial = await evaluate(client, `(() => {
    ...
    return {
      ...
      actionButtons: root?.querySelectorAll('[data-action="app-primary"], [data-action="second-brain-capture"], [data-action="second-brain-link"], [data-action="second-brain-training-pack"], [data-action="second-brain-public-contributor-pack"], [data-action="second-brain-obsidian-starter-vault"], [data-action="second-brain-ai-council-pack"], [data-action="second-brain-obsidian-graph-map"], [data-action="second-brain-review"], [data-action="second-brain-export-github"]').length || 0,
  ...
  ensure(initial.actionButtons === 10, `expected ten Second Brain actions, got ${initial.actionButtons}`);
  ...
  const artifacts = await evaluate(client, `(() => {
    ...
    return {
      ...
      obsidianGraphMapVisible: text.includes('Last Obsidian Graph Map') && !text.includes('Not built yet'),
  ...
  ensure(artifacts.notePaths.length >= 12, `Second Brain expected snapshot plus note/review/contributor/Obsidian starter/AI council/graph map markdown files, got ${artifacts.notePaths.length}`);
