...
const files = {
  ...
  datasetCard: 'SEIS_UNIVERSE_MEMORY_RANKER_DATASET_CARD.md',
  modelCard: 'SEIS_UNIVERSE_MEMORY_RANKER_MODEL_CARD.md',
  dataset: 'packages/seis-ai/data/memory-ranker-seed-v0.json',
  source: 'packages/seis-ai/src/model/memory-ranker-lab.mjs',
  artifact: 'packages/seis-ai/models/memory-ranker-seed-v0.json',
  test: 'packages/seis-ai/test/memory-ranker-lab.test.mjs',
...
for (const [file, token] of [
  [files.family, '`seis-memory-ranker`'],
  [files.family, 'embedding/reranking experiment'],
  [files.datasetCard, '# SEIS Universe Memory Ranker Dataset Card'],
  [files.datasetCard, '`seis-memory-ranker-seed-v0`'],
  [files.modelCard, '# SEIS Universe Memory Ranker Model Card'],
  [files.modelCard, '`seis-memory-ranker-seed-v0`'],
  [files.source, 'trainMemoryRankerModel'],
  [files.source, 'buildMemoryRankerModelArtifact'],
  [files.test, 'SEIS Universe memory ranker learning lab'],
  [files.packageJson, '"check:seis-universe-memory-ranker-model"'],
]) {
  requireIncludes(file, token);
}

const dataset = loadMemoryRankerDataset(path.join(root, files.dataset));
...
if (!report.ok) {
  failures.push(`memory ranker eval failed: ${JSON.stringify(report.failed)}`);
}

const expectedArtifact = JSON.stringify(buildMemoryRankerModelArtifact(dataset), null, 2);
const actualArtifact = read(files.artifact).trim();
if (actualArtifact !== expectedArtifact) {
  failures.push('model artifact is stale; run npm run automation:seis-memory-ranker-model');
}
...
if (failures.length > 0) {
  console.error('SEIS Universe memory ranker model check failed:');
...
console.log('SEIS Universe memory ranker model check passed.');
