  ...
  const record = {
    ...
    waves: [
      ...
      },
      {
        ...
        currentEvidencePath: WAVE_4_PROGRAM_PATH,
      ...
      },
      {
        ...
        plannedProgramPath: WAVE_4_PROGRAM_PATH,
        totalSteps: 100,
...
function validateRecord(record) {
  ...
  assert(list(record.waves).length === 5 && record.waves[0]?.status === "completed" && record.waves[0]?.completedSteps === 100 && record.waves[1]?.status === "completed" && record.waves[1]?.completedSteps === 100 && record.waves[2]?.status === "in-progress" && record.waves[2]?.completedSteps === 99 && list(record.waves[2]?.inProgressSteps).join(",") === "100" && record.waves[2]?.priorValidationPath === WAVE_3_FINAL_VALIDATION_PATH && record.waves[2]?.preflightPath === WAVE_3_FINAL_PREFLIGHT_PATH && record.waves[2]?.deliveryEvidencePath === WAVE_3_DELIVERY_EVIDENCE_PATH && record.waves[2]?.repositoryLocalHandoffPath === WAVE_3_REPOSITORY_LOCAL_HANDOFF_PATH && record.waves[2]?.followingWaveReviewPath === WAVE_3_FOLLOWING_WAVE_REVIEW_PATH && record.waves[2]?.currentEvidencePath === WAVE_4_PROGRAM_PATH && record.waves[3]?.status === "planned-gated" && record.waves[3]?.plannedProgramPath === WAVE_4_PROGRAM_PATH && record.waves[3]?.scopeReviewPath === WAVE_3_FOLLOWING_WAVE_REVIEW_PATH && record.waves[3]?.selectedCapability === "seis-swift-package-topology" && record.waves[3]?.activationApproved === false && record.waves[3]?.implementationStarted === false && record.waves[3]?.candidatePackageExists === false && record.waves[3]?.candidatePublicCardExists === false && record.waves[3]?.totalSteps === 100 && record.waves[4]?.status === "planned-gated" && record.waves[4]?.totalSteps === 100, "wave states are invalid");
