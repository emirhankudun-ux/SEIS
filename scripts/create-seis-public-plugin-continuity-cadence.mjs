...
function buildRecord() {
  ...
  assert(wave4Program?.id === "seis-public-plugin-wave-4-program" && wave4Program?.status === "in-progress" && wave4Program?.maturity === "prototype" && wave4Program?.wave?.number === 4 && wave4Program?.scope?.selectedCapability === "seis-swift-package-topology" && wave4Program?.activationGate?.status === "implemented-repository-local" && wave4Program?.activationGate?.activationDecisionPath === WAVE_4_ACTIVATION_DECISION_PATH && wave4Program?.activationGate?.implementationStarted === true && wave4Program?.activationGate?.candidatePackageExists === true && wave4Program?.activationGate?.candidatePublicCardExists === true && wave4Program?.progress?.completedStepCount === 95 && list(wave4Program?.progress?.inProgressStepNumbers).join(",") === "96" && wave4Program?.evidence?.integrationCheckpointPath === WAVE_4_INTEGRATION_CHECKPOINT_PATH && wave4Program?.evidence?.validationDeliveryEvidencePath === WAVE_4_VALIDATION_DELIVERY_EVIDENCE_PATH && wave4Program?.evidence?.publicBoundaryDecisionPath === WAVE_4_PUBLIC_BOUNDARY_DECISION_PATH && wave4Program?.evidence?.handoffPreparationPath === WAVE_4_HANDOFF_PREPARATION_PATH && wave4Program?.evidence?.closeoutSequenceDecisionPath === WAVE_4_CLOSEOUT_SEQUENCE_DECISION_PATH, "Wave 4 handoff-preparation state is invalid");
  assert(wave4TopologyEvidence?.id === "seis-swift-package-topology" && wave4TopologyEvidence?.status === "ready-public-static-topology-evidence" && wave4TopologyEvidence?.marketplace?.applicationPluginCount === 74 && wave4TopologyEvidence?.marketplace?.publicCardCount === 380 && wave4TopologyEvidence?.activation?.implementationObserved === true && wave4TopologyEvidence?.audit?.ok === true && wave4TopologyEvidence?.safety?.compilesSwift === false && wave4TopologyEvidence?.safety?.runsSwiftTests === false && wave4TopologyEvidence?.publicBoundary?.personalMarketplaceRead === false && wave4TopologyEvidence?.publicBoundary?.personalMarketplaceMutation === false, "Wave 4 topology evidence is invalid");
  ...
  assert(wave4HandoffPreparation?.id === "seis-public-plugin-wave-4-handoff-preparation" && wave4HandoffPreparation?.status === "in-progress-repository-local-handoff-preparation" && wave4HandoffPreparation?.step === 96 && wave4HandoffPreparation?.stateAtPreparation?.completedStepCount === 95 && wave4HandoffPreparation?.stateAtPreparation?.activeStep === 96 && list(wave4HandoffPreparation?.stateAtPreparation?.remainingStepNumbers).join(",") === range(97, 100).join(",") && Object.values(wave4HandoffPreparation?.completedEvidence || {}).every(Boolean) && wave4HandoffPreparation?.handoffGate?.ready === false && wave4HandoffPreparation?.handoffGate?.allOneHundredStepsHaveCurrentEvidence === false && wave4HandoffPreparation?.handoffGate?.terminalHandoffPublished === false && wave4HandoffPreparation?.handoffGate?.waveCompleted === false && wave4HandoffPreparation?.handoffGate?.wave5ActivationApproved === false && Object.values(wave4HandoffPreparation?.externalClaims || {}).every((value) => value === false) && wave4HandoffPreparation?.recommendedFollowUp?.status === "created-proposed-owner-decision-required" && wave4HandoffPreparation?.recommendedFollowUp?.decisionPath === WAVE_4_CLOSEOUT_SEQUENCE_DECISION_PATH, "Wave 4 handoff preparation is invalid");
  ...
  const record = {
    ...
    cadence: {
      ...
      waveSeries: {
        ...
        activeWave: 4,
        activeWaveState: "repository-local-public-boundary-decision-complete-step-96-in-progress",
      },
      githubDelivery: {
    ...
    waves: [
      ...
      },
      {
        ...
        currentEvidencePath: WAVE_4_PUBLIC_BOUNDARY_DECISION_PATH,
        predecessor: "Wave 3 repository-local handoff",
        entryRule: "Wave 3 closed with current evidence, the separate activation decision approved one fixed-manifest scope, and the package completed its first repository-local integration checkpoint without a release claim.",
      },
      {
        wave: 5,
...
function validateRecord(record) {
  ...
  assert(list(record.waves).length === 5 && record.waves[0]?.status === "completed" && record.waves[0]?.completedSteps === 100 && record.waves[1]?.status === "completed" && record.waves[1]?.completedSteps === 100 && record.waves[2]?.status === "completed" && record.waves[2]?.completedSteps === 100 && list(record.waves[2]?.inProgressSteps).length === 0 && record.waves[2]?.priorValidationPath === WAVE_3_FINAL_VALIDATION_PATH && record.waves[2]?.preflightPath === WAVE_3_FINAL_PREFLIGHT_PATH && record.waves[2]?.deliveryEvidencePath === WAVE_3_DELIVERY_EVIDENCE_PATH && record.waves[2]?.repositoryLocalHandoffPath === WAVE_3_REPOSITORY_LOCAL_HANDOFF_PATH && record.waves[2]?.followingWaveReviewPath === WAVE_3_FOLLOWING_WAVE_REVIEW_PATH && record.waves[2]?.closeoutPath === WAVE_3_CLOSEOUT_PATH && record.waves[2]?.currentEvidencePath === WAVE_3_CLOSEOUT_PATH && record.waves[3]?.status === "in-progress" && record.waves[3]?.programPath === WAVE_4_PROGRAM_PATH && record.waves[3]?.scopeReviewPath === WAVE_3_FOLLOWING_WAVE_REVIEW_PATH && record.waves[3]?.activationDecisionPath === WAVE_4_ACTIVATION_DECISION_PATH && record.waves[3]?.selectedCapability === "seis-swift-package-topology" && record.waves[3]?.activationApproved === true && record.waves[3]?.implementationStarted === true && record.waves[3]?.candidatePackageExists === true && record.waves[3]?.candidatePublicCardExists === true && record.waves[3]?.completedSteps === 95 && list(record.waves[3]?.inProgressSteps).join(",") === "96" && record.waves[3]?.topologyEvidencePath === WAVE_4_TOPOLOGY_EVIDENCE_PATH && record.waves[3]?.integrationCheckpointPath === WAVE_4_INTEGRATION_CHECKPOINT_PATH && record.waves[3]?.validationDeliveryEvidencePath === WAVE_4_VALIDATION_DELIVERY_EVIDENCE_PATH && record.waves[3]?.publicBoundaryDecisionPath === WAVE_4_PUBLIC_BOUNDARY_DECISION_PATH && record.waves[3]?.handoffPreparationPath === WAVE_4_HANDOFF_PREPARATION_PATH && record.waves[3]?.closeoutSequenceDecisionPath === WAVE_4_CLOSEOUT_SEQUENCE_DECISION_PATH && record.waves[3]?.currentEvidencePath === WAVE_4_PUBLIC_BOUNDARY_DECISION_PATH && record.waves[3]?.totalSteps === 100 && record.waves[4]?.status === "planned-gated" && record.waves[4]?.totalSteps === 100, "wave states are invalid");
