...
function buildRecord() {
  ...
  assert(wave4Program?.id === "seis-public-plugin-wave-4-program" && wave4Program?.status === "in-progress" && wave4Program?.maturity === "prototype" && wave4Program?.wave?.number === 4 && wave4Program?.scope?.selectedCapability === "seis-swift-package-topology" && wave4Program?.activationGate?.status === "implemented-repository-local" && wave4Program?.activationGate?.activationDecisionPath === WAVE_4_ACTIVATION_DECISION_PATH && wave4Program?.activationGate?.implementationStarted === true && wave4Program?.activationGate?.candidatePackageExists === true && wave4Program?.activationGate?.candidatePublicCardExists === true && wave4Program?.progress?.completedStepCount === 97 && list(wave4Program?.progress?.inProgressStepNumbers).join(",") === "98" && wave4Program?.closeoutSequence?.status === "approved-owner-mapping-applied" && list(wave4Program?.closeoutSequence?.completedSteps).join(",") === "96,97" && wave4Program?.closeoutSequence?.activeStep === 98 && wave4Program?.repositoryLocalHandoff?.status === "completed-repository-local-handoff" && wave4Program?.repositoryLocalHandoff?.activeStep === 98 && wave4Program?.evidence?.integrationCheckpointPath === WAVE_4_INTEGRATION_CHECKPOINT_PATH && wave4Program?.evidence?.validationDeliveryEvidencePath === WAVE_4_VALIDATION_DELIVERY_EVIDENCE_PATH && wave4Program?.evidence?.publicBoundaryDecisionPath === WAVE_4_PUBLIC_BOUNDARY_DECISION_PATH && wave4Program?.evidence?.handoffPreparationPath === WAVE_4_HANDOFF_PREPARATION_PATH && wave4Program?.evidence?.closeoutSequenceDecisionPath === WAVE_4_CLOSEOUT_SEQUENCE_DECISION_PATH && wave4Program?.evidence?.repositoryLocalHandoffPath === WAVE_4_REPOSITORY_LOCAL_HANDOFF_PATH, "Wave 4 repository-local handoff state is invalid");
  ...
  const record = {
    ...
    cadence: {
      ...
      waveSeries: {
        ...
        activeWave: 4,
        activeWaveState: "repository-local-handoff-complete-step-98-in-progress",
      },
      githubDelivery: {
    ...
    waves: [
      ...
      },
      {
        ...
        currentEvidencePath: WAVE_4_REPOSITORY_LOCAL_HANDOFF_PATH,
        predecessor: "Wave 3 repository-local handoff",
        entryRule: "Wave 3 closed with current evidence, the separate activation decision approved one fixed-manifest scope, and Wave 4 completed a non-published repository-local handoff; step 98 remains a separate planned-gate review and is not a release claim.",
      },
      {
        wave: 5,
