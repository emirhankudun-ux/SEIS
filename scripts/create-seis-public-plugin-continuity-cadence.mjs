  ...
  const record = {
    ...
    cadence: {
      ...
      waveSeries: {
        ...
        activeWave: null,
        activeWaveState: "wave-4-completed-wave-5-planned-gated",
      },
      githubDelivery: {
    ...
    waves: [
      ...
      },
      {
        ...
        entryRule: "Activate only after Wave 4 closes with current validation, a Wave 5-specific scope and risk decision, an explicit activation decision, and current user authority.",
      },
    ],
    futureWaveTemplate,
    rolloutRules: [
...
function validateRecord(record) {
  ...
  assert(list(record.waves).length === 5 && record.waves[0]?.status === "completed" && record.waves[0]?.completedSteps === 100 && record.waves[1]?.status === "completed" && record.waves[1]?.completedSteps === 100 && record.waves[2]?.status === "completed" && record.waves[2]?.completedSteps === 100 && list(record.waves[2]?.inProgressSteps).length === 0 && record.waves[2]?.priorValidationPath === WAVE_3_FINAL_VALIDATION_PATH && record.waves[2]?.preflightPath === WAVE_3_FINAL_PREFLIGHT_PATH && record.waves[2]?.deliveryEvidencePath === WAVE_3_DELIVERY_EVIDENCE_PATH && record.waves[2]?.repositoryLocalHandoffPath === WAVE_3_REPOSITORY_LOCAL_HANDOFF_PATH && record.waves[2]?.followingWaveReviewPath === WAVE_3_FOLLOWING_WAVE_REVIEW_PATH && record.waves[2]?.closeoutPath === WAVE_3_CLOSEOUT_PATH && record.waves[2]?.currentEvidencePath === WAVE_3_CLOSEOUT_PATH && record.waves[3]?.status === "completed" && record.waves[3]?.programPath === WAVE_4_PROGRAM_PATH && record.waves[3]?.scopeReviewPath === WAVE_3_FOLLOWING_WAVE_REVIEW_PATH && record.waves[3]?.activationDecisionPath === WAVE_4_ACTIVATION_DECISION_PATH && record.waves[3]?.selectedCapability === "seis-swift-package-topology" && record.waves[3]?.activationApproved === true && record.waves[3]?.implementationStarted === true && record.waves[3]?.candidatePackageExists === true && record.waves[3]?.candidatePublicCardExists === true && record.waves[3]?.completedSteps === 100 && list(record.waves[3]?.inProgressSteps).length === 0 && record.waves[3]?.topologyEvidencePath === WAVE_4_TOPOLOGY_EVIDENCE_PATH && record.waves[3]?.integrationCheckpointPath === WAVE_4_INTEGRATION_CHECKPOINT_PATH && record.waves[3]?.validationDeliveryEvidencePath === WAVE_4_VALIDATION_DELIVERY_EVIDENCE_PATH && record.waves[3]?.publicBoundaryDecisionPath === WAVE_4_PUBLIC_BOUNDARY_DECISION_PATH && record.waves[3]?.handoffPreparationPath === WAVE_4_HANDOFF_PREPARATION_PATH && record.waves[3]?.closeoutSequenceDecisionPath === WAVE_4_CLOSEOUT_SEQUENCE_DECISION_PATH && record.waves[3]?.repositoryLocalHandoffPath === WAVE_4_REPOSITORY_LOCAL_HANDOFF_PATH && record.waves[3]?.followingWaveReviewPath === WAVE_4_FOLLOWING_WAVE_REVIEW_PATH && record.waves[3]?.evidenceRetentionPath === WAVE_4_EVIDENCE_RETENTION_PATH && record.waves[3]?.closeoutPath === WAVE_4_CLOSEOUT_PATH && record.waves[3]?.currentEvidencePath === WAVE_4_CLOSEOUT_PATH && record.waves[3]?.totalSteps === 100 && record.waves[4]?.status === "planned-gated" && record.waves[4]?.candidateReviewPath === WAVE_4_FOLLOWING_WAVE_REVIEW_PATH && record.waves[4]?.selectedCapability === "seis-plugin-capability-coverage" && record.waves[4]?.implementationApproved === false && record.waves[4]?.activationApproved === false && record.waves[4]?.totalSteps === 100, "wave states are invalid");
