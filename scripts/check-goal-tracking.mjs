...
const requirementMatrixPath = "content/development/seis-goal-requirement-matrix.json";
...
const requiredDocs = [
  ...
  "docs/reviews/GOAL_TRACKING_MONTHLY_REVIEW_2026-06.md",
  ...
  "docs/reviews/GOAL_TRACKING_REQUIREMENT_MATRIX.md"
];
...
if (!existsSync(requirementMatrixPath)) {
  failures.push(`missing goal requirement matrix: ${requirementMatrixPath}`);
}
...
const requirementMatrix = existsSync(requirementMatrixPath)
  ? JSON.parse(readFileSync(requirementMatrixPath, "utf8"))
  : null;
...
if (requirementMatrix) {
  validateRequirementMatrix(requirementMatrix);
}
...
console.log(JSON.stringify({
  ...
  requirementMatrixRecords: requirementMatrix.records.length,
...
function validateRequirementMatrix(matrix) {
  if (matrix.schemaVersion !== 1) {
    failures.push("goal requirement matrix schemaVersion must be 1");
  }

  if (matrix.mode !== "non_llm_goal_requirement_matrix") {
    failures.push("goal requirement matrix mode must be non_llm_goal_requirement_matrix");
  }
  ...
  if (!Array.isArray(records) || records.length < 20) {
    failures.push("goal requirement matrix must include detailed requirement records");
  }

  for (const record of records) {
    const label = record.id || "(missing requirement id)";
    for (const field of [
      "id",
      "requirement_group",
      "requirement",
    ...
    if (ids.has(record.id)) {
      failures.push(`duplicate requirement matrix id: ${record.id}`);
    }
    ids.add(record.id);

    if (!/^SEIS-REQ-\d{3}$/.test(record.id || "")) {
      failures.push(`${label} id must match SEIS-REQ-000 format`);
    }
    ...
    for (const field of ["evidence_ids", "related_goal_ids", "related_paths", "gaps"]) {
      if (!Array.isArray(record[field])) {
        failures.push(`${label} ${field} must be an array`);
      }
    }
    ...
    for (const gap of record.gaps || []) {
      validateSafeText(label, "gaps", gap);
    }
    ...
    if ((record.status === "partial" || record.status === "blocked" || record.status === "planned") && !record.gaps?.length) {
      failures.push(`${label} must name gaps for non-proved status`);
    }
  }
}
...
function validateCommandCenterView(view) {
  ...
  if (!registry || !evidenceLedger || !executionRegistry || !reviewCadenceRegistry || !reviewLogRegistry || !planningHorizonsRegistry || !progressLedger || !objectiveCoverage || !completionGate || !requirementMatrix) {
  ...
  for (const source of [registryPath, evidencePath, executionPath, reviewCadencePath, reviewLogPath, planningHorizonsPath, progressLedgerPath, objectiveCoveragePath, completionGatePath, requirementMatrixPath]) {
  ...
  if (view.summary?.totalRequirementMatrixRecords !== requirementMatrix.records.length) {
    failures.push(`${label} totalRequirementMatrixRecords does not match requirement matrix`);
  }
  ...
  if (!view.panels?.requirementMatrix?.length) {
    failures.push(`${label} must expose requirement matrix`);
  }
