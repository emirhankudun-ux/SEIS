...
async function smokeDesktopSharedVfs(client, baseUrl) {
  ...
  ensure(installedAiSystems.mcpRuntimeDiagnostics.resourceCount === 31, `Desktop MCP Runtime Contract diagnostics expected 31 resources, got ${installedAiSystems.mcpRuntimeDiagnostics.resourceCount}`);
...
async function smokeSubAgentFiveYearDemo(client, baseUrl) {
  ...
  ensure(initial.constellationText.includes("35 MCP tools") && initial.constellationText.includes("31 resources"), "SEIS demo constellation inspector missing MCP contract counts");
  ...
  ensure(initial.constellationInspector?.mcpRuntimeToolCount === 35, `SEIS demo constellation MCP tool count mismatch: ${initial.constellationInspector?.mcpRuntimeToolCount}`);
  ensure(Number(initial.constellationInspector?.mcpRuntimeResourceCount) === 31, `SEIS demo constellation MCP resource count mismatch: ${initial.constellationInspector?.mcpRuntimeResourceCount}`);
  ...
  ensure(initial.hero3dDiagnostics?.mcpRuntimeToolCount === 35, `SEIS demo 3D MCP tool diagnostic count mismatch: ${initial.hero3dDiagnostics?.mcpRuntimeToolCount}`);
  ensure(Number(initial.hero3dDiagnostics?.mcpRuntimeResourceCount) === 31, `SEIS demo 3D MCP resource diagnostic count mismatch: ${initial.hero3dDiagnostics?.mcpRuntimeResourceCount}`);
  ...
  ensure(initial.hero3dStatus.includes("6 AI routes") && initial.hero3dStatus.includes("5 plugin lanes") && initial.hero3dStatus.includes("35 MCP tools"), `SEIS demo 3D hero status missing installed AI/plugin/MCP count: ${initial.hero3dStatus}`);
  ...
  ensure(afterExport.mcpRuntimeToolCount === 35, `SEIS demo evidence report expected 35 MCP tools, got ${afterExport.mcpRuntimeToolCount}`);
  ensure(Number(afterExport.mcpRuntimeResourceCount) === 31, `SEIS demo evidence report expected 31 MCP resources, got ${afterExport.mcpRuntimeResourceCount}`);
  ...
  ensure(afterExport.constellationInspectorMcpToolCount === 35, `SEIS demo evidence constellation MCP tool count mismatch: ${afterExport.constellationInspectorMcpToolCount}`);
  ensure(Number(afterExport.constellationInspectorMcpResourceCount) === 31, `SEIS demo evidence constellation MCP resource count mismatch: ${afterExport.constellationInspectorMcpResourceCount}`);
