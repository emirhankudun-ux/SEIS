...
async function smokeDesktopSharedVfs(client, baseUrl) {
  ...
  ensure(installedAiSystems.mcpRuntimeDiagnostics.toolCount === 37, `Desktop MCP Runtime Contract diagnostics expected 37 tools, got ${installedAiSystems.mcpRuntimeDiagnostics.toolCount}`);
...
async function smokeSubAgentFiveYearDemo(client, baseUrl) {
  ...
  ensure(initial.constellationText.includes("37 MCP tools") && initial.constellationText.includes("30 resources"), "SEIS demo constellation inspector missing MCP contract counts");
  ...
  ensure(initial.constellationInspector?.mcpRuntimeToolCount === 37, `SEIS demo constellation MCP tool count mismatch: ${initial.constellationInspector?.mcpRuntimeToolCount}`);
  ...
  ensure(initial.hero3dDiagnostics?.mcpRuntimeToolCount === 37, `SEIS demo 3D MCP tool diagnostic count mismatch: ${initial.hero3dDiagnostics?.mcpRuntimeToolCount}`);
  ...
  ensure(initial.hero3dStatus.includes("6 AI routes") && initial.hero3dStatus.includes("5 plugin lanes") && initial.hero3dStatus.includes("37 MCP tools"), `SEIS demo 3D hero status missing installed AI/plugin/MCP count: ${initial.hero3dStatus}`);
  ...
  ensure(afterExport.mcpRuntimeToolCount === 37, `SEIS demo evidence report expected 37 MCP tools, got ${afterExport.mcpRuntimeToolCount}`);
  ...
  ensure(afterExport.constellationInspectorMcpToolCount === 37, `SEIS demo evidence constellation MCP tool count mismatch: ${afterExport.constellationInspectorMcpToolCount}`);
  ensure(Number(afterExport.constellationInspectorMcpResourceCount) === 30, `SEIS demo evidence constellation MCP resource count mismatch: ${afterExport.constellationInspectorMcpResourceCount}`);
