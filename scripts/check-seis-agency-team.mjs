...
const sourceRegistryPath = "content/development/seis-agent-registry.json";
...
validateSources(registry, sourceRegistry, goalTracking);
validateRolesAndPods(registry, sourceRegistry);
...
console.log(
  "SEIS agency team check passed: 5 pods, 13 source-backed roles, "
...
function validateSources(value, source, goals) {
  ensureObject(source, "source agent registry");
  ...
  const sourceRoles = source.inventories?.secondBrainAgentRoles?.records;
  ensure(Array.isArray(sourceRoles), "source registry Second Brain roles are missing");
  const sourceNames = sourceRoles.map((record) => record?.agent);
  ensure(sourceNames.length === expectedRoleNames.length, "source registry role count must remain 13");
  ensure(
    arraysEqual([...sourceNames].sort(), [...expectedRoleNames].sort()),
    "agency source roles must cover the existing 13 Second Brain role names exactly"
  );
...
function validateRolesAndPods(value, source) {
  ...
  const sourceNames = new Set(source.inventories.secondBrainAgentRoles.records.map((record) => record.agent));
  ...
  for (const role of value.roles) {
    ensure(sourceNames.has(role.sourceRole), "role source is not in the canonical roster: " + role.sourceRole);
