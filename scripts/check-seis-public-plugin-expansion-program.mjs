...
for (let index = 0; index < 5; index += 1) {
  ...
  if (index === 0) {
    ...
    ensure(Array.isArray(wave3Program?.steps) && wave3Program.steps.length === 100 && wave3Program.steps.every((step, stepIndex) => step?.status === (stepIndex < 98 ? "completed" : stepIndex === 98 ? "in-progress" : "planned")), "wave 3 step state is invalid");
...
ensure(Array.isArray(continuityCadence?.waves) && continuityCadence.waves.length === 5 && continuityCadence.waves[2]?.completedSteps === 98 && Array.isArray(continuityCadence.waves[2]?.inProgressSteps) && continuityCadence.waves[2].inProgressSteps[0] === 99 && continuityCadence.waves[2]?.currentEvidencePath === "content/development/seis-public-plugin-wave-3-following-wave-review.json" && Array.isArray(continuityCadence?.futureWaveTemplate?.steps) && continuityCadence.futureWaveTemplate.steps.length === 100, "continuity cadence wave evidence is invalid");
