...
function inspectStaticReadiness({ bundleCatalog, selectionGuide, generalAutopilot, supervisedAutopilot, unifiedSuite, failures }) {
  ...
  ensure(supervisedAutopilot?.immediateCycle?.totalSteps === 30 && immediateRounds.length === 5 && immediateRounds.every((round) => Array.isArray(round?.steps) && round.steps.length === 6), "hardened runner bootstrap contract is invalid", failures);
  ensure(supervisedAutopilot?.fiveWaveSeries?.waves === 5 && supervisedAutopilot?.fiveWaveSeries?.stepsPerWave === 100, "autopilot must retain five 100-step waves", failures);
  ensure(escalationTiers.length === 5 && escalationTiers.map((tier) => tier?.stepsPerWave).join(",") === "200,300,400,500,600", "autopilot escalation tiers are invalid", failures);
  ...
  ensure(generalAutopilot?.canonicalAutomation?.goalId === "SEIS-GOAL-0025" && generalAutopilot?.canonicalAutomation?.reviewedPhaseCount === 48 && generalAutopilot?.canonicalAutomation?.repositoryAnchored === true && generalAutopilot?.commandAllowlist === undefined, "general-plugin roadmap must delegate to the hardened Goal 0025 runner", failures);
