...
const evidence = collectSeisCorePluginChangeEvidence(root, { threshold: SEIS_CORE_PLUGIN_CHANGE_EVIDENCE_THRESHOLD });
...
if (checkMode) {
  ...
  console.log(JSON.stringify({ ok: true, id: evidence.id, baseCommit: evidence.baseCommit, codeLinesChanged: evidence.codeLinesChanged, eligible: evidence.eligible }, null, 2));
  process.exit(0);
}
