...
function buildRecord() {
  ...
  assert(followUpDecision?.id === "seis-public-plugin-wave-2-follow-up-decision" && followUpDecision?.status === "completed-no-additional-public-plugin-selected" && followUpDecision?.decision?.selectedCapability === null && followUpDecision?.decision?.outcome === "no-additional-public-plugin-selected" && followUpDecision?.swiftPmEvidence?.localToolingObservation?.swiftTestCompletionClaim === "not-completed-and-not-claimed" && followUpDecision?.swiftPmEvidence?.validationBoundary?.testPassClaim === false && (followUpDecision?.publicDistribution?.additionalCardAdded === false || followUpDecision?.publicDistribution?.currentDirectCardAdded === false) && followUpDecision?.publicDistribution?.personalMarketplaceRead === false && followUpDecision?.publicDistribution?.personalMarketplaceMutation === false && followUpDecision?.publicDistribution?.publicReleaseAllowed === false, "Wave 2 follow-up decision is invalid");
  ...
  const record = {
    ...
    nonGoals: [
      ...
      "Running SwiftPM from a public plugin runtime, or running a simulator, native application, signing, provisioning, App Store submission, deployment, provider calls, or a public release.",
      "Treating static source evidence or an interrupted manual local SwiftPM attempt as live native capability proof.",
