...
if (contract) {
  ...
  ensure(typeof contract.remote?.url === "string" && contract.remote.url.includes("github.com/"), "contract remote URL must target GitHub");
  ensure(typeof contract.remote?.targetBranch === "string" && contract.remote.targetBranch.length > 0, "contract target branch must be explicit");
  ensure((contract.remote?.acceptedLocalBranches || []).includes(contract.remote?.targetBranch), "contract must accept the target branch as a publishing branch");
