...
@Test func aiCommandCoreEngineRoutesBuildRequestsAndScoresRisk() {
    ...
    #expect(run.workflow.contains { $0.id == "approval" && $0.state == "Waiting" })
    #expect(run.providerReadiness.contains { $0.id == "browser-secrets" && $0.state == "Blocked" })
    #expect(run.metrics.contains { $0.id == "providers" && $0.note == "Live calls disabled" })
